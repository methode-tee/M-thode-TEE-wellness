-- MÉTHODE TEE — V4896594 — backfill buckets 0 et 1
begin;
...
set profile=jsonb_set(
    ...
    updated_at=now()
where mod(abs(hashtext(coalesce(p.profile_key,p.dictionary_id::text,p.ciqual_code::text,p.display_name,''))::bigint),8) in (0,1);
commit;
