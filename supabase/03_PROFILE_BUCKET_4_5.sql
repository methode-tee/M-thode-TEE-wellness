-- MÉTHODE TEE — V4896594 — backfill buckets 4 et 5
begin;
...
set profile=jsonb_set(
    ...
    updated_at=now()
where mod(abs(hashtext(coalesce(p.profile_key,p.dictionary_id::text,p.ciqual_code::text,p.display_name,''))::bigint),8) in (4,5);
commit;
