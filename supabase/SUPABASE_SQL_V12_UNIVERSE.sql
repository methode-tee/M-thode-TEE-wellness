-- MÉTHODE TEE V12 — Univers privé complet
...
create policy "posts_read_paid_admin" on public.posts for select using (
  active = true and (public.is_admin() or exists(select 1 from public.profiles p where p.id = auth.uid() and p.has_app_access = true))
);
...
drop policy if exists "pages_read_paid_admin" on public.app_pages;
create policy "pages_read_paid_admin" on public.app_pages for select using (active = true or public.is_admin());
drop policy if exists "pages_admin_manage" on public.app_pages;
create policy "pages_admin_manage" on public.app_pages for all using (public.is_admin()) with check (public.is_admin());

drop policy if exists "sections_read_paid_admin" on public.page_sections;
create policy "sections_read_paid_admin" on public.page_sections for select using (active = true or public.is_admin());
drop policy if exists "sections_admin_manage" on public.page_sections;
create policy "sections_admin_manage" on public.page_sections for all using (public.is_admin()) with check (public.is_admin());
...
create policy "contents_owned_admin" on public.protocol_contents for select using (
  public.is_admin() or exists(select 1 from public.user_protocols up where up.protocol_id = protocol_contents.protocol_id and up.user_id = auth.uid() and up.status='active')
);
...
insert into storage.buckets (id, name, public) values ('page-media','page-media',true) on conflict(id) do update set public=true;
insert into storage.buckets (id, name, public) values ('protocol-media','protocol-media',true) on conflict(id) do update set public=true;
insert into storage.buckets (id, name, public) values ('protocol-files','protocol-files',true) on conflict(id) do update set public=true;

drop policy if exists "media_public_read" on storage.objects;
create policy "media_public_read" on storage.objects for select using (bucket_id in ('post-media','page-media','protocol-media','protocol-files'));

drop policy if exists "media_admin_insert" on storage.objects;
create policy "media_admin_insert" on storage.objects for insert with check (bucket_id in ('post-media','page-media','protocol-media','protocol-files') and public.is_admin());

drop policy if exists "media_admin_update" on storage.objects;
create policy "media_admin_update" on storage.objects for update using (bucket_id in ('post-media','page-media','protocol-media','protocol-files') and public.is_admin()) with check (bucket_id in ('post-media','page-media','protocol-media','protocol-files') and public.is_admin());

drop policy if exists "media_admin_delete" on storage.objects;
create policy "media_admin_delete" on storage.objects for delete using (bucket_id in ('post-media','page-media','protocol-media','protocol-files') and public.is_admin());
...
('profil','Profil','Profil','👤','dashboard',5,true)
...
select 'maux-de-ventre','Maux de ventre','Digestion lourde','pharmacie_vegetale','🤢','Digestion lourde, ventre gonflé, inconfort après repas.','Ajoute ici PDFs, vidéos, checklists, tracker, calendrier et routines depuis l’admin.',500,'5 jours',true
