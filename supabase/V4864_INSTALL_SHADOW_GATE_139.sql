-- MÉTHODE TEE — V486.4 · SHADOW GATE SUR LE CORPUS V485
-- Installe un évaluateur parallèle qui exécute LES MÊMES 139 attentes critiques
-- contre resolve_food_speech_phrase_v8_json, sans modifier le gate V485.3 historique.
...
begin
  if to_regclass('public.food_speech_regression_cases') is null then
    raise exception 'V486.4 shadow gate nécessite le corpus V485';
  end if;
  if to_regprocedure('public.resolve_food_speech_phrase_v8_json(text,jsonb,integer)') is null then
    raise exception 'V486.4 shadow gate nécessite le resolver V5';
  end if;
end
$preflight$;

create or replace function public.food_speech_regression_eval_v4864(p_case_id bigint)
returns jsonb
language plpgsql
volatile
security invoker
...
begin
  ...
  v_payload := public.resolve_food_speech_phrase_v8_json(c.phrase,c.choices,12);
...
-- 4. RUNNER DU CORPUS
-- ---------------------------------------------------------------------------
create or replace function public.food_speech_regression_run_v4864(
...
begin
  ...
  loop
    e := public.food_speech_regression_eval_v4864(c.id);
...
-- ---------------------------------------------------------------------------
create or replace function public.food_speech_regression_gate_v4864()
returns jsonb
language sql
volatile
security invoker
...
as $v485_gate$
  with r as materialized (
    select * from public.food_speech_regression_run_v4864(null,true)
  ), cat as (
    select category,
           count(*) total,
  ...
  select jsonb_build_object(
    'baseline','V485.3',
    'corpus_version','V485 + V486.4 shadow',
...
-- ---------------------------------------------------------------------------
create or replace function public.food_speech_regression_assert_v4864()
returns jsonb
language plpgsql
volatile
security invoker
...
declare
  g jsonb;
begin
  g := public.food_speech_regression_gate_v4864();
...
revoke all on function public.food_speech_regression_eval_v4864(bigint) from anon, authenticated;
revoke all on function public.food_speech_regression_run_v4864(text,boolean) from anon, authenticated;
revoke all on function public.food_speech_regression_gate_v4864() from anon, authenticated;
revoke all on function public.food_speech_regression_assert_v4864() from anon, authenticated;
