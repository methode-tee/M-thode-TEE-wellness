...
returns jsonb language sql stable security invoker set search_path=public as $$
  ...
  calculable as (select * from items where quantity_g is not null and quantity_g>0 and ciqual_code is not null),
  ...
  macros as (select case when count(*)>0 then sum(protein) else null end protein_g,case when count(*)>0 then sum(fiber) else null end fiber_g from calculable),
  micros as (
    select e.key, sum(case when jsonb_typeof(e.value)='number' then (e.value#>>'{}')::numeric else nullif(e.value->>'value','')::numeric end) value
  ...
  select jsonb_build_object(
    ...
    'protein_g',m.protein_g,'fiber_g',m.fiber_g,
