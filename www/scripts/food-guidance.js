...
(function(){
  ...
  function profileGoalMode(model,payload){
    ...
    const canonical=[profile?.main_goal,settings?.body_intention,model?.bodyIntent].map(x=>String(x||'').trim()).find(Boolean)||'';
  ...
  function hydrationContext(model,payload){
    ...
    const referenceLiters=firstNumeric(core.hydration_liters,core.beverage_hydration_liters);
  ...
  async function fetchBreakfastRoleCandidates(group,date=localDate()){
    ...
    try{
      const result=await rpcVersioned(['mt_food_breakfast_role_candidates_v5','mt_food_breakfast_role_candidates_v4','mt_food_breakfast_role_candidates_v3','mt_food_breakfast_role_candidates_v2','mt_food_breakfast_role_candidates_v1'],{p_role:role,p_target_date:date,p_limit:48},'TEE breakfast roles');
  ...
  async function ensureDinnerAddons(payload,state,date=localDate()){
    if(String(state?.mealContext||'')!=='dinner')return payload;
  ...
  function humanizeStoredMealName(name){
    ...
    if(/^fruit de la passion ou maracudja/.test(t))return 'Fruit de la passion';
    if(/^galette de cereale s soufflee s aux graines/.test(t)||/^galette de cereales soufflees aux graines/.test(t))return 'Galettes de céréales soufflées aux graines';
    if(/^galette de cereales soufflees$/.test(t))return 'Galettes de céréales soufflées';
    if(/^tartine croustillante au seigle/.test(t))return 'Tartine croustillante au seigle';
    if(/^baie de goji sechee/.test(t))return 'Baies de goji';
    if(/^cassis cru/.test(t))return 'Cassis';
    if(/^sureau baie crue/.test(t))return 'Baies de sureau';
    if(/^pavot graine/.test(t))return 'Graines de pavot';
    if(/^tortilla souple .* base de ble/.test(t))return 'Tortilla de blé';
    if(/^tortilla souple .* base de mais/.test(t))return 'Tortilla de maïs';
    if(/^cereales pour petit dejeuner tres riches en fibres/.test(t))return 'Céréales riches en fibres';
    if(/^petales de mais nature/.test(t))return 'Pétales de maïs nature';
    if(/^figue de barbarie/.test(t))return 'Figue de Barbarie';
    if(/^tamarin fruit mur/.test(t)||/^tamarin /.test(t))return 'Tamarin';
    if(/^anone ou cherimole/.test(t))return 'Chérimole';
    if(/^longan /.test(t))return 'Longan';
    if(/^ramboutan ou rambutan/.test(t))return 'Ramboutan';
    if(/^fruit du jacquier/.test(t))return 'Fruit du jacquier';
    return raw;
  }
  ...
  function trustedConfirmedBreakfastPortion(c){
    return c?.habitual_portion_confident===true||String(c?.habitual_portion_confident||'').toLowerCase()==='true';
  }
  function realisticBreakfastCandidate(c,group){
    ...
    if(curated&&!trustedConfirmedBreakfastPortion(c)){
      ...
      let target=current;
      if(minG!==null&&target<minG)target=minG;
      if(maxG!==null&&target>maxG)target=maxG;
      if(Math.abs(target-current)>=.5)return rescaleCandidatePortion(c,target,'tee_breakfast_curated_range');
      return c;
    }
  ...
  function diversifyBreakfastRoleCandidates(rows){
    // V4896662 : même principe que le dîner — aucune suppression, simple intercalage
    // des familles pour que « Voir d'autres… » ne soit pas rempli de variantes quasi identiques.
    const queues=new Map(),order=[],out=[];
    for(const c of rows||[]){
      const key=String(c?.breakfast_family_key||c?.breakfast_rotation_family||c?.rotation_family_curated||c?.candidate_ref||c?.name||'other');
      if(!queues.has(key)){queues.set(key,[]);order.push(key);}
      queues.get(key).push(c);
    }
    let moved=true;
  ...
  function guidanceFoodFamily(c){
    ...
    if(/(^| )(tofu|tempeh|seitan|whey|proteine de chanvre|proteine vegetale|protéine végétale|protein powder)( |$)/.test(t))return 'plant_protein';
    ...
    if(/(^| )(skyr|yaourt|yogourt|yogurt|yoplait|fromage blanc|petit suisse|lait|fromage|feta|mozzarella|ricotta)( |$)/.test(t))return 'dairy';
  ...
  function mealRoleGroup(c){
    ...
    if(String(c?.breakfast_curated_version||'').startsWith('V4896663')){
      ...
      if(curatedRole==='fruit_fiber')return 'side';
      if(curatedRole==='beverage')return 'beverage';
      if(curatedRole==='micro_addon')return 'accent';
      if(curatedRole==='non_proactive')return 'accent';
    }
  ...
  function snackRoleFitScore(c,group,model,payload,state){
    ...
    if(group==='protein'){
      ...
      if(['fruit','starch'].includes(family))score-=14;
      if(family==='fat_side')score-=40;
      if(/(^| )(pavot|tournesol|chia|lin|sesame|graine|amande|noix|noisette|cajou|pistache)( |$)/.test(name))score-=24;
      if(/(^| )(whey|proteine|protéine)( |$)/.test(name)&&portion>50)score-=180;
      const ratio=protein>0?kcal/protein:99;if(ratio>22)score-=Math.min(35,(ratio-22)*2.2);
      if(protein<5)score-=24;
      if(g.goal.key==='mass_gain'||g.goal.key==='recomposition')score+=Math.min(10,protein*.45);
    }else if(group==='starch'){
      score+=Math.min(32,kcal/18)+Math.min(25,carbs*.75);
      if(family==='starch')score+=26;
      else if(family==='fruit')score+=13;
      else if(family==='dairy'||family==='fat_side')score+=7;
      if(kcal<60)score-=18;
      if(g.goal.key==='mass_gain')score+=Math.min(12,kcal/45);
    }else if(group==='side'){
      score+=fiber*5.2;
      if(family==='fruit')score+=28;
      else if(family==='legumes')score+=18;
      else if(family==='vegetable')score+=14;
      else if(family==='fat_side')score+=4;
      if(fiber<1.5)score-=20;
      if(g.digestiveCaution&&fiber>8)score-=10;
    }
    if(level==='habit')score+=18;else if(level==='consumed')score+=11;
  ...
  function addMealBuildChoice(state,c){
    const ctx=String(state?.mealContext||''),data=loadMealBuildState(state),group=mealRoleGroup(c),structuredSnack=ctx==='snack'&&!!c?.__tee_structured_group,item={key:String(c?.candidate_ref||c?.dictionary_id||c?.ciqual_code||c?.name||'Option'),candidate_ref:c?.candidate_ref||null,ciqual_code:c?.ciqual_code||c?.code||null,dictionary_id:c?.dictionary_id||c?.food_dictionary_id||null,name:candidateDisplayName(c),group,role:mealIntegrationRole(c),family:guidanceFoodFamily(c),structured_snack:structuredSnack,portion_g:n(c?.portion_g),preparation_state:preparationState(c),kcal:n(c?.kcal),protein_g:n(c?.protein_g),fiber_g:n(c?.fiber_g),carbs_g:n(c?.carbs_g),fat_g:n(c?.fat_g)};
    if(['breakfast','lunch','dinner'].includes(ctx)||structuredSnack){
      data.items=(data.items||[]).filter(x=>x?.group!==group);
      data.items.push(item);
      if(ctx==='breakfast'||ctx==='dinner')data.addon=null;
    }else{
  ...
  function removeMealBuildChoice(state,token){
    const ctx=String(state?.mealContext||''),data=loadMealBuildState(state),structuredSnack=ctx==='snack'&&(data.items||[]).some(x=>x?.structured_snack===true);
    if(['breakfast','lunch','dinner'].includes(ctx)||structuredSnack){
  ...
  function dinnerAddonPer100(c,focus){
    if(focus==='protein')return n(c?.protein_100g)||0;
    if(focus==='fiber')return n(c?.fiber_100g)||0;
    return n(c?.kcal_100g)||0;
  }
  function dinnerBuildTags(build){
    const tags=new Set(['complete_dinner']),items=Array.isArray(build?.items)?build.items:[];
    for(const x of items){
      const name=normText(x?.name),family=String(x?.family||''),group=String(x?.group||'');
      if(group==='protein')tags.add('has_protein');
      if(group==='starch'||family==='starch')tags.add('has_starch');
      if(group==='vegetable'||family==='vegetable')tags.add('has_vegetable');
      if(family==='dairy'||/(^| )(yaourt|yogourt|yogurt|skyr|fromage blanc|petit suisse|faisselle|ricotta)( |$)/.test(name))tags.add('has_dairy');
      if(family==='fruit'||/(^| )(pomme|poire|banane|orange|kiwi|mangue|ananas|fraise|framboise|myrtille|fruit de la passion)( |$)/.test(name))tags.add('has_fruit');
      if(/(^| )(amande|noix|noisette|cajou|pistache|pignon|cacahuete|arachide)( |$)/.test(name))tags.add('has_nuts');
      if(/(^| )(chia|lin|tournesol|courge|sesame|pavot).*(graine|graines)/.test(name))tags.add('has_seeds');
      if(/(^| )(brie|camembert|cantal|cheddar|comte|emmental|feta|gouda|gruyere|mozzarella|parmesan|reblochon|roquefort|chevre)( |$)/.test(name))tags.add('has_cheese');
      if(String(x?.role||'')==='treat'||/(chocolat|dessert|gateau|glace|profiterole)/.test(name))tags.add('has_sweet');
    }
    return tags;
  }
  function dinnerAddonCompatible(c,build,need){
    if(!c||c?.dinner_addon_enabled!==true||!need||addonAlreadyInBuild(c,build))return false;
    const mode=String(c?.dinner_addon_mode||'never');if(mode==='never')return false;
    if(mode==='familiar_only'&&Number(c?.context_use_count_60d||0)<1)return false;
    const needs=Array.isArray(c?.dinner_addon_need_tags)?c.dinner_addon_need_tags:[];if(needs.length&&!needs.includes(need.focus))return false;
    const tags=dinnerBuildTags(build),pairs=Array.isArray(c?.dinner_addon_pairing_tags)?c.dinner_addon_pairing_tags:[],avoid=Array.isArray(c?.dinner_addon_avoid_tags)?c.dinner_addon_avoid_tags:[];
    if(pairs.length&&!pairs.every(tag=>tags.has(String(tag||''))))return false;
    if(avoid.some(tag=>tags.has(String(tag||''))))return false;
    return true;
  }
  function dinnerAddonPortion(c,need){
    const minG=n(c?.dinner_addon_portion_min_g),typicalG=n(c?.dinner_addon_portion_typical_g),maxG=n(c?.dinner_addon_portion_max_g);
    if(!minG||!maxG||!need)return null;
    if(c?.habitual_portion_confident===true&&n(c?.habitual_portion_median_g)>0)return Math.max(1,Math.round(n(c.habitual_portion_median_g)));
    const per100=dinnerAddonPer100(c,need.focus),target=need.focus==='energy'?clamp(need.remaining*.18,90,300):need.focus==='protein'?clamp(need.remaining*.55,6,18):clamp(need.remaining*.72,2.5,7);
    let grams=typicalG||minG;if(per100>0)grams=clamp((target/per100)*100,minG,maxG);grams=Math.round(grams/5)*5;return clamp(grams,minG,maxG);
  }
  function scaledDinnerAddon(c,need){
    const grams=dinnerAddonPortion(c,need);if(!grams)return null;const scale=grams/100,out={...c,portion_g:grams,addon_focus:need.focus};
    out.kcal=(n(c?.kcal_100g)||0)*scale;out.protein_g=(n(c?.protein_100g)||0)*scale;out.fiber_g=(n(c?.fiber_100g)||0)*scale;out.carbs_g=(n(c?.carbs_100g)||0)*scale;out.fat_g=(n(c?.fat_100g)||0)*scale;return out;
  }
  function dinnerAddonContributionEnough(c,need){
    if(!c||!need)return false;const amount=need.focus==='protein'?n(c?.protein_g)||0:need.focus==='fiber'?n(c?.fiber_g)||0:n(c?.kcal)||0;
    return amount>=(need.focus==='energy'?55:need.focus==='protein'?3:1.2);
  }
  function dinnerAddonScore(c,build,need,model,payload,state){
    const amount=need.focus==='energy'?(n(c?.kcal)||0):need.focus==='protein'?(n(c?.protein_g)||0):(n(c?.fiber_g)||0),target=need.focus==='energy'?clamp(need.remaining*.18,100,300):need.focus==='protein'?clamp(need.remaining*.55,6,18):clamp(need.remaining*.72,2.5,7),ratio=amount/Math.max(1,target),uses=Number(c?.context_use_count_60d||0),priority=Number(c?.dinner_addon_priority||0),kind=String(c?.dinner_addon_kind||''),goal=profileGoalMode(model,payload),global=globalNutritionState(model,payload,state);let score=priority*8;
    score+=clamp(ratio,0,1.25)*30;if(ratio>1.75)score-=Math.min(12,(ratio-1.75)*7);
    if(uses>=2)score+=24;else if(uses===1)score+=14;
    if(c?.habitual_portion_confident===true)score+=12;
    if(goal.key==='mass_gain'&&need.focus==='energy'&&['nuts','dairy','cheese','dried_fruit'].includes(kind))score+=7;
    if(global.digestiveCaution&&['nuts','dried_fruit','seed_crunch'].includes(kind))score-=7;
  ...
  function dinnerAddonReason(need,c=null){
    ...
    if(need?.focus==='protein')return hint||`Les protéines restent encore un peu basses après ce dîner. Tee propose un seul complément curé, sans rajouter une deuxième base protéinée.`;
    if(need?.focus==='fiber')return hint||`Les fibres restent encore un peu basses après ce dîner. Tee propose un seul complément curé si cela reste pertinent.`;
    return hint||'Petit complément facultatif, seulement s’il complète réellement ce dîner.';
  }
  function dinnerAddonHTML(payload,model,build,state){
    ...
    return `<div class="mt-food-guide-addon"><div class="mt-food-guide-addon-kicker">Petit complément facultatif</div><div class="mt-food-guide-addon-row"><div class="mt-food-guide-addon-copy"><b>${esc(candidateDisplayName(c))} · ${esc(fmt(c.portion_g,0))} g</b><span>${esc(dinnerAddonReason(pick.need,c))}${metrics?` ${esc(metrics)}.`:''}</span></div><button type="button" class="mt-food-guide-addon-action" data-mt-guide-add-evening-addon>Ajouter</button></div></div>`;
  }
  function saveDinnerAddon(state,c,need){
    const data=loadMealBuildState(state);data.addon={key:String(c?.profile_key||c?.candidate_ref||c?.dictionary_id||c?.ciqual_code||c?.display_name||'evening-addon'),candidate_ref:c?.candidate_ref||null,profile_key:c?.profile_key||null,ciqual_code:c?.ciqual_code||null,dictionary_id:c?.dictionary_id||null,name:candidateDisplayName(c),portion_g:n(c?.portion_g),kcal:n(c?.kcal),protein_g:n(c?.protein_g),fiber_g:n(c?.fiber_g),kind:c?.dinner_addon_kind||'evening_optional',addon_focus:need?.focus||null};saveMealBuildState(state,data);return data;
  }
  ...
  function breakfastInterdayRotationTier(c){
    const serverTier=Number(c?.breakfast_rotation_rank_tier);
    if(Number.isFinite(serverTier)&&serverTier>=0)return serverTier;
    ...
    if(confirmedExact>0)return 6;
    if(chosenExact>0)return 5;
    if(confirmedFamily>0)return 4;
    if(chosenFamily>0)return 3;
    if(shown>0)return 1;
    return 0;
  }
  function breakfastInterdayRankScore(c,model,focus,state){
    // V4896662 : le malus est réellement intégré au score final, pas seulement exposé en métadonnée.
    // Il reste souple : l'aliment demeure dans les alternatives et peut remonter si le contexte le justifie.
    return contextualCandidateScore(c,model,focus,state)-breakfastInterdayRotationPenalty(c)*3;
  }
  ...
  function candidateHTML(c,focus,index,state,opts={}){
    ...
    const ctx=String(state?.mealContext||''),mealStructured=opts?.structured===true||(['breakfast','lunch','dinner'].includes(ctx)&&opts?.structured!==false)||(ctx==='snack'&&opts?.structured!==false&&!!c?.__tee_structured_group);
  ...
  function renderHost(host,{model,decision,payload,experience=false,start=0,browseRole=null,animateStep=false}){
    ...
    const snackPlan=ctx==='snack'&&structuredMainMeal?initialSnackPlan:[],snackReady=ctx==='snack'&&structuredMainMeal&&snackPlan.length>0&&snackPlan.every(g=>selectedMealGroups.has(g));
    ...
    const isBreakfastStructured=ctx==='breakfast',isSnackStructured=ctx==='snack'&&structuredMainMeal;
    ...
    const candidateMarkup=visible.map((c,i)=>{const idx=(browseRole?i:start+i),group=mealRoleGroup(c),hasAlt=structuredMainMeal&&!browseRole&&candidates.length>1;return candidateHTML(c,focus,idx,state,{showRoleAlt:hasAlt,structured:structuredMainMeal});}).join('');
    ...
    host.querySelector('[data-mt-guide-more]')?.addEventListener('click',()=>{const section=host.querySelector('.mt-food-guide-options');if(!section)return;section.innerHTML=candidates.slice(0,Math.min(9,candidates.length)).map((c,i)=>candidateHTML(c,focus,i,state,{structured:structuredMainMeal})).join('');bindPickButtons(section);host.querySelector('[data-mt-guide-more]')?.remove();});
    ...
    host.querySelector('[data-mt-guide-add-evening-addon]')?.addEventListener('click',async()=>{const pick=selectDinnerAddon(payload,model,build,state);if(!pick)return;saveDinnerAddon(state,pick.c,pick.need);await log('chosen',pick.need?.focus||focus,pick.c,{mealContext:'dinner',payload:{intent_only:true,evening_optional_addon:true,dinner_addon_curated:true,dinner_addon_kind:pick.c?.dinner_addon_kind||null,profile_key:pick.c?.profile_key||null,portion_g:pick.c?.portion_g,remaining_focus:pick.need?.focus||null,remaining_gap:pick.need?.remaining??null}});renderHost(host,{model,decision,payload,experience,start:0,browseRole:null});});
