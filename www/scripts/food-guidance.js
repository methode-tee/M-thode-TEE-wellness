/* MÉTHODE TEE · V4896626 · reveal Jardin intérieur uniquement sur la nouvelle étape contextuelle
...
(function(){
  ...
  function injectCSS(){if(document.getElementById('mtFoodGuidanceCSS'))return;const s=document.createElement('style');s.id='mtFoodGuidanceCSS';s.textContent=`
    ...
    .mt-food-guide-transition{display:flex;align-items:center;gap:10px;margin:11px 0 2px;padding:10px 12px;border-radius:13px;background:rgba(255,250,242,.78);border:1px solid #eadfc9;color:#587168;font-size:11.5px;font-weight:750;line-height:1.35;opacity:0;transform:translateY(5px);animation:mtGuideTransitionIn .28s cubic-bezier(.22,.61,.36,1) forwards;transition:opacity .14s ease,transform .14s ease}.mt-food-guide-transition.is-leaving{opacity:0!important;transform:translateY(-3px)!important}.mt-food-guide-transition-mark{width:7px;height:7px;border-radius:999px;background:#a77f35;box-shadow:0 0 0 4px rgba(167,127,53,.09);flex:0 0 auto}.mt-food-guide-transition-dots{display:inline-flex;gap:3px;margin-left:auto;flex:0 0 auto}.mt-food-guide-transition-dots i{width:4px;height:4px;border-radius:99px;background:#a77f35;opacity:.28;animation:mtGuideDot 1s ease-in-out infinite}.mt-food-guide-transition-dots i:nth-child(2){animation-delay:.14s}.mt-food-guide-transition-dots i:nth-child(3){animation-delay:.28s}.mt-food-guide-next-step.reveal{opacity:0;transform:translateY(24px);transition:opacity .65s ease,transform .65s ease;will-change:opacity,transform}.mt-food-guide-next-step.reveal.visible{opacity:1;transform:translateY(0)}@keyframes mtGuideTransitionIn{to{opacity:1;transform:translateY(0)}}@keyframes mtGuideDot{0%,70%,100%{opacity:.22;transform:translateY(0)}35%{opacity:.9;transform:translateY(-2px)}}@media (prefers-reduced-motion:reduce){.mt-food-guide-transition,.mt-food-guide-transition-dots i{animation:none!important;opacity:1!important;transform:none!important}.mt-food-guide-next-step.reveal{transition:none!important;transform:none!important;opacity:1!important}}
    ...
    const bindPickButtons=(root)=>{
      root.querySelectorAll('[data-mt-guide-pick]').forEach(btn=>btn.addEventListener('click',async()=>{
        ...
        if(structuredMainMeal){
          ...
          if(elapsed<240)await sleep(240-elapsed);
