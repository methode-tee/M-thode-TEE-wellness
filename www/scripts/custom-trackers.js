...
(function(){
  ...
  function trackerDailySummary(rawKey,values={},settings={},date=TODAY()){
    ...
    if(key==='sommeil_profond'){
      ...
      metric('Repas calculables',calculated===null?'':mealCount!==null?`${calculated}/${mealCount}`:calculated);metric('Protéines',protein===null?'':`${protein} g`);metric('Fibres',fiber===null?'':`${fiber} g`);metric('Micronutriments documentés',coverage);metric('Profil',profile);
      signals.nutrition_protein_g=protein;signals.nutrition_fiber_g=fiber;signals.micronutrient_coverage_count=coverage;signals.nutrition_calculated_meals=calculated;signals.nutrition_meal_count=mealCount;signals.nutrition_data_coverage=calculated!==null&&mealCount!==null&&mealCount>0?Math.round(calculated/mealCount*100):null;signals.nutrition_data_quality=values._nutrition_data_quality||null;signals.energy=num(values.energy);signals.appetite=num(values.appetite);signals.digestion=num(values.digestion);signals.nutrition_micros=values._micronutrients||null;
    }else if(key==='cycle'){
  ...
  function hourlyCard(rows){const latest=[...rows].find(row=>parseHourly(row.values?._healthkit_hourly_steps).length);if(!latest)return null;const hours=parseHourly(latest.values._healthkit_hourly_steps),bins=Array.from({length:24},()=>0);hours.forEach(x=>{let h=Number(x.hour);if(!Number.isFinite(h)&&x.start){const d=new Date(x.start);if(!Number.isNaN(d.getTime()))h=d.getHours();}if(h>=0&&h<24)bins[h]+=Number(x.steps)||0;});const total=bins.reduce((a,b)=>a+b,0);if(!total)return null;const before14=bins.slice(0,14).reduce((a,b)=>a+b,0),pct=Math.round(before14/total*100);return {type:'hourly',label:'Répartition de la marche',value:`${pct} % avant 14 h`,detail:'Les périodes sans pas ne sont pas qualifiées automatiquement de sédentaires.',source:'Apple Santé',bins,aria:`Répartition des pas : ${pct} pour cent avant 14 heures`};}
  ...
  function trackerAnalytics(key,rows,settings={}){
    const cards=[],add=x=>{if(x)cards.push(x);};
    if(key==='sommeil_profond'){add(rangeCard('Durée du sommeil',rows,v=>num(v._sleep_hours)??durationBetween(v.bedtime,v.wake_time),'h','Saisi / Apple Santé',v=>formatDuration(v)));add(ratingCard('Qualité ressentie',rows,v=>num(v.quality)));add(ratingCard('État au réveil',rows,v=>num(v.wake_state)));add(frequencyCard('Nuits avec réveils',rows,v=>num(v.awakenings)>0));add(consistencyCard('Régularité de durée',rows,v=>num(v._sleep_hours)??durationBetween(v.bedtime,v.wake_time),'h'));}
    else if(key==='digestion'){add(ratingCard('Confort digestif',rows,v=>num(v.comfort)));add(ratingCard('Ballonnements',rows,v=>num(v.bloating)));add(ratingCard('Douleurs / crampes',rows,v=>num(v.pain)));add(frequencyCard('Jours avec gêne',rows,v=>/gêne|inconfort|variable/i.test(String(v.day_state||''))));add(distributionCard('Moments les plus renseignés',rows,v=>v.meal_moment));}
    else if(key==='reflux'){add(frequencyCard('Jours avec reflux',rows,v=>v.episode&&!/^Non/i.test(String(v.episode))));add(ratingCard('Intensité ressentie',rows,v=>/^Non/i.test(String(v.episode||''))?0:num(v.intensity)));add(distributionCard('Moment / contexte',rows,v=>v.meal_gap_bed));add(distributionCard('Position après repas',rows,v=>v.position));}
    else if(key==='equilibre_alimentaire'){add(ratingCard('Diversité',rows,v=>num(v.diversity)));add(ratingCard('Repères protéines',rows,v=>num(v.protein)));add(ratingCard('Végétaux',rows,v=>num(v.plants)));add(ratingCard('Satiété',rows,v=>num(v.satiety_after)));}
    else if(key==='evolution_corporelle'){if(!settings.hide_weight)add(rangeCard('Poids',rows,v=>num(v.weight),'kg'));add(rangeCard('Tour de taille',rows,v=>num(v.waist),'cm'));add(rangeCard('Tour de hanches',rows,v=>num(v.hips),'cm'));add(rangeCard('Masse grasse',rows,v=>num(v.body_fat),'%'));add(rangeCard('Masse maigre',rows,v=>num(v.lean_body_mass),'kg'));add(ratingCard('Confort corporel',rows,v=>num(v.body_comfort)));}
    else if(key==='peau'){add(ratingCard('Imperfections',rows,v=>num(v.blemishes)));add(ratingCard('Réactivité / rougeurs',rows,v=>avg(v.inflammation,v.sensitivity)));add(ratingCard('Sécheresse',rows,v=>num(v.dryness)));add(frequencyCard('Jours avec changement',rows,v=>/réactive|changement/i.test(String(v.day_state||''))));}
    else if(key==='performance_recuperation'){add(ratingCard('Récupération',rows,v=>num(v.recovery)));add(ratingCard('Disponibilité',rows,v=>num(v.readiness)??num(v.availability)));add(ratingCard('Charge ressentie',rows,v=>num(v.intensity)));add(rangeCard('Temps de pratique',rows,v=>num(v.duration),'min'));add(frequencyCard('Jours avec pratique',rows,v=>v.session&&!/Repos/i.test(String(v.session))));}
    else if(key==='cycle'){add(ratingCard('Énergie',rows,v=>num(v.energy)));add(ratingCard('Sommeil',rows,v=>num(v.sleep)));add(ratingCard('Douleurs',rows,v=>num(v.pain)));add(ratingCard('Stress',rows,v=>num(v.stress)));add(frequencyCard('Jours avec flux renseigné',rows,v=>present(v.flow)&&v.flow!=='Aucun'));}
    else if(key==='perimenopause'){add(ratingCard('Sommeil',rows,v=>num(v.sleep)));add(ratingCard('Énergie',rows,v=>num(v.energy)));add(ratingCard('Stress',rows,v=>num(v.stress)));add(frequencyCard('Jours avec bouffées',rows,v=>v.hot_flashes&&!/^Aucune/i.test(String(v.hot_flashes))));}
    else if(key==='jeune_intermit'){add(rangeCard('Durée du jeûne',rows,v=>num(v._fast_hours)??durationBetween(v.last_meal,v.first_meal),'h','Saisi manuellement',v=>formatDuration(v)));add(ratingCard('Faim',rows,v=>num(v.hunger)));add(ratingCard('Énergie',rows,v=>num(v.energy)));add(ratingCard('Confort après rupture',rows,v=>num(v.break_quality)));add(frequencyCard('Jours de pause',rows,v=>/pause/i.test(String(v.fast_state||''))));}
    else if(key==='reduction_sucre'){add(ratingCard('Intensité des envies',rows,v=>num(v.craving)??semanticScore(v.craving_state,{Aucune:0,Légère:3,Présente:6,Forte:9,Variable:5})));add(frequencyCard('Jours sans sucre ajouté',rows,v=>v.no_added_sugar==='Oui'));add(frequencyCard('Alternatives utiles',rows,v=>v.alternative_help==='Oui'||v.alternative_help==='Un peu'));add(distributionCard('Moments des envies',rows,v=>v.moment));add(distributionCard('Déclencheurs renseignés',rows,v=>v.trigger));}
    else if(key==='changer_habitude'){add(frequencyCard('Petits pas réalisés',rows,v=>/Petit pas réalisé/i.test(String(v.day_state||''))||present(v.victory)));add(ratingCard('Confiance',rows,v=>num(v.confidence)));add(ratingCard('Difficulté / friction',rows,v=>num(v.difficulty)));add(ratingCard('Intensité de l’impulsion',rows,v=>num(v.urge)));}
    else if(key==='pas_marche'){add(rangeCard('Pas',rows,v=>num(v.steps),'pas',rows.some(r=>present(r.values?._healthkit_source))?'Apple Santé / saisie':'Saisi manuellement'));add(rangeCard('Distance',rows,v=>num(v.distance_km),'km'));add(rangeCard('Longueur de pas',rows,v=>num(v.step_length_cm),'cm',rows.some(r=>present(r.values?._healthkit_source))?'Apple Santé / saisie':'Saisi manuellement'));add(rangeCard('Vitesse de marche',rows,v=>num(v.walking_speed_kmh),'km/h'));add(consistencyCard('Régularité du rythme',rows,v=>num(v.steps),'pas'));if(settings.goal_mode==='Fixer mon propre objectif')add(progressCard('Repère personnel',rows,v=>num(v.steps),Number(settings.step_goal),'pas'));add(hourlyCard(rows));}
    else if(key==='nutrition_vegetale'){add(rangeCard('Protéines calculées',rows,v=>num(v.protein_g),'g','Carnet · CIQUAL'));add(rangeCard('Fibres calculées',rows,v=>num(v.fiber_g),'g','Carnet · CIQUAL'));add(rangeCard('Micronutriments documentés',rows,v=>num(v.micronutrient_coverage_count),'','Carnet · CIQUAL'));add(frequencyCard('Jours avec repas calculables',rows,v=>num(v.calculated_meals)>0,'Carnet · CIQUAL'));const coverage=metricSeries(rows,v=>{const a=num(v.calculated_meals),b=num(v.meal_count);return a!==null&&b>0?a/b*100:null;});if(coverage.length){const value=average(coverage.map(x=>x.value));add({type:'ratingPercent',label:'Données nutritionnelles exploitables',value:`${Math.round(value)} %`,detail:'Qualité du journal, pas qualité de l’alimentation',source:'Carnet · CIQUAL',percent:value,aria:`Données nutritionnelles exploitables : ${Math.round(value)} pour cent`});}}
    return {cards:cards.slice(0,8)};
  }
  function analyticsCardHTML(card){const meter=['rating','frequency','consistency','progress','ratingPercent','distributionText'].includes(card.type)?`<div class="mt-follow-gauge-track" aria-hidden="true"><div class="mt-follow-gauge-fill" style="width:${Math.max(0,Math.min(100,Number(card.percent)||0))}%"></div></div>`:'';let visual=meter;if(card.type==='range'&&Number.isFinite(card.min)&&Number.isFinite(card.max)){const span=card.max-card.min||1,pos=Math.max(0,Math.min(100,(card.latest-card.min)/span*100));visual=`<div class="mt-follow-range-track" aria-hidden="true"><i style="left:${pos}%"></i></div>`;}if(card.type==='hourly'){const max=Math.max(1,...card.bins);visual=`<div class="mt-follow-distribution" aria-hidden="true">${card.bins.map(v=>`<i style="height:${Math.max(3,Math.round(v/max*100))}%"></i>`).join('')}</div>`;}return `<article class="mt-follow-analytics-card" role="group" aria-label="${esc(card.aria||card.label)}"><small>${esc(card.label)}</small><b>${esc(card.value)}</b>${visual}<p>${esc(card.detail||'')}</p><p style="margin-top:5px;color:#a28e73">${esc(card.source||'')}</p></article>`;}
  function analyticsHTML(key,rows,settings,days){const model=trackerAnalytics(key,rows,settings);if(!model.cards.length)return `<div class="mt-follow-source-note">Les jauges apparaîtront dès qu’un premier repère compatible sera renseigné. Une donnée absente reste inconnue, jamais égale à zéro.</div>`;const label=!Number(days)?'Depuis le début':`${days} jours`;return `<section class="mt-follow-analytics"><div class="mt-follow-analytics-title"><h3>Mes jauges</h3><small>${esc(label)} · données réellement renseignées</small></div><div class="mt-follow-analytics-grid">${model.cards.map(analyticsCardHTML).join('')}</div><p class="mt-follow-analytics-note">Ces jauges sont descriptives : elles montrent plages, moyennes, fréquences ou repères choisis. Elles ne posent aucun diagnostic et ne transforment pas les jours sans donnée en zéro.</p></section>`;}
  ...
  function renderHistory(modal,key,rows,days=7){
    ...
    modal.dataset.key=key;modal.dataset.period=String(days);modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" type="button" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">${esc(item.title)} · évolution</div><h2>Mes jauges et mon évolution</h2><div class="mt-follow-history-head"><button class="mt-follow-period ${days===7?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(7)">7 j</button><button class="mt-follow-period ${days===28?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(28)">28 j</button><button class="mt-follow-period ${days===90?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(90)">90 j</button><button class="mt-follow-period ${!Number(days)?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(0)">Depuis le début</button><button class="mt-follow-period" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">+ Aujourd’hui</button></div>${analyticsHTML(key,shown,preference(key).settings||{},days)}<div class="mt-follow-history-stats"><div class="mt-follow-stat"><b>${stats.all?stats.count:`${stats.count}/${days}`}</b><small>${stats.all?'repères renseignés':'jours renseignés'}</small></div><div class="mt-follow-stat"><b>${stats.average===null?'—':String(Math.round(stats.average*10)/10).replace('.',',')}</b><small>${esc(stats.label)}</small></div></div>${highlightHTML}${sparklineHTML(key,shown)}<div class="mt-follow-insight"><small>Lecture prudente · ${esc(periodLabel)}</small><b>${esc(stats.trend)}</b><p>${esc(stats.relation||'Continue quelques repères comparables pour faire ressortir une tendance personnelle utile.')} ${esc(used)}</p></div>${key==='evolution_corporelle'?'<div class="mt-follow-source-note">Les variations de poids, mensurations ou composition corporelle sont décrites sans jugement. Compare de préférence des mesures prises dans des conditions proches.</div>':''}${historyRowsHTML(key,shown)}<button class="mt-follow-secondary" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">Renseigner aujourd’hui</button></section>`;
  }
  window.mtAdvancedTrackerHistory=async function(rawKey){
    ...
    const modal=root('mtAdvancedTrackerEntry','mt-follow-entry');modal.dataset.key=key;modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" type="button" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-loading"><b>${esc(item.title)}</b><p>Lecture de tes repères enregistrés…</p><span></span></div></section>`;modal.classList.add('open');
    renderHistory(modal,key,await fetchHistory(key,7),7);
  };
  window.mtAdvancedTrackerHistoryPeriod=async function(days){const modal=root('mtAdvancedTrackerEntry','mt-follow-entry'),key=normalizeKey(modal.dataset.key),n=[0,7,28,90].includes(Number(days))?Number(days):7;modal.querySelector('.mt-follow-sheet')?.classList.add('is-loading');renderHistory(modal,key,await fetchHistory(key,n),n);};
  ...
  async function saveEntry(event){
    ...
    if(key==='nutrition_vegetale'&&settings.diet_pattern&&!values.diet_pattern)values.diet_pattern=settings.diet_pattern;
