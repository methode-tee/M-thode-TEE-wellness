...
(function(){
  ...
  async function globalTrackerRows(days=28){const n=Math.min(90,Math.max(7,Number(days)||28)),key=`${UID||'guest'}:${n}`,cached=GLOBAL_TRENDS_CACHE.get(key);if(cached&&Date.now()-cached.at<5*60*1000)return cached.rows;const c=client();if(!c||!UID)return[];const from=addDays(TODAY(),-(n-1));try{const q=c.from('user_tracker_entries').select('tracker_key,entry_date,values').eq('user_id',UID).gte('entry_date',from).lte('entry_date',TODAY()).order('entry_date',{ascending:false}).limit(360),r=await Promise.race([q,new Promise(resolve=>setTimeout(()=>resolve({data:[]}),2800))]),rows=Array.isArray(r?.data)?r.data:[];GLOBAL_TRENDS_CACHE.set(key,{at:Date.now(),rows});return rows;}catch(_){return[];}}
  ...
  async function temporalRefluxTrend(days=28,trackerRows=[]){const reflux=trackerRows.filter(r=>r.tracker_key==='reflux'&&!/^Non/i.test(String(r.values?.episode||''))&&r.values?.onset);if(reflux.length<3)return null;const c=client();if(!c||!UID)return null;const from=addDays(TODAY(),-(Math.min(90,days)-1));try{const q=c.from('food_meals').select('meal_date,meal_time').eq('user_id',UID).gte('meal_date',from).lte('meal_date',TODAY()).not('meal_time','is',null).order('meal_date',{ascending:false}).limit(120),r=await Promise.race([q,new Promise(resolve=>setTimeout(()=>resolve({data:[]}),2200))]),meals=Array.isArray(r?.data)?r.data:[];let matched=0;reflux.forEach(ep=>{const [oh,om]=String(ep.values.onset).slice(0,5).split(':').map(Number),o=oh*60+om;const same=meals.filter(m=>m.meal_date===ep.entry_date&&m.meal_time).map(m=>String(m.meal_time).slice(0,5)).map(t=>{const [h,mi]=t.split(':').map(Number);return h*60+mi}).filter(mm=>o>=mm&&o-mm<=180);if(same.length)matched++;});if(matched<3)return null;return {title:'Repas → reflux : ordre temporel observé',copy:`${matched} épisode${matched>1?'s':''} de reflux sur ${reflux.length} renseigné${reflux.length>1?'s':''} sont survenus dans les 3 h suivant un repas enregistré. Cela ne prouve pas qu’un aliment précis en est la cause.`};}catch(_){return null;}}
  window.mtAdvancedTrackerGlobalTrends=async function(days=28){addCSS();if(!UID)UID=(await getUser())?.id||window.__MT_LIBRARY_USER_ID__||null;const modal=root('mtAdvancedTrackerEntry','mt-follow-entry');modal.classList.add('open');modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">Méthode Tee · transversal</div><h2><img class="mt-official-star-inline" src="assets/brand-compass-star-transparent.png" alt="" aria-hidden="true">Mes tendances</h2><div class="mt-follow-loading"><b>Lecture de mes repères</b><p>Une seule lecture bornée, uniquement parce que tu viens d’ouvrir cette vue.</p><span></span></div></section>`;const rows=await globalTrackerRows(days),cards=[];const sleepEnergy=relationTrend('Sommeil ↔ énergie','Les journées avec une durée de sommeil différente voient aussi l’énergie renseignée évoluer',pairByDate(rows,'sommeil_profond',crossSleep,['performance_recuperation','cycle','perimenopause','stress_regulation'],crossEnergy));if(sleepEnergy)cards.push(['Corps',sleepEnergy]);const walkMood=relationTrend('Marche ↔ humeur','Les journées avec davantage de pas voient aussi l’humeur renseignée évoluer',pairByDate(rows,'pas_marche',v=>num(v.steps),['stress_regulation','cycle','perimenopause'],v=>num(v.mood)));if(walkMood)cards.push(['Mental',walkMood]);const stressSleep=relationTrend('Stress ↔ sommeil','Le stress renseigné et la qualité de sommeil semblent évoluer ensemble',pairByDate(rows,'stress_regulation',v=>num(v.stress),['sommeil_profond'],v=>num(v.quality)));if(stressSleep)cards.push(['Mental',stressSleep]);const temporal=await temporalRefluxTrend(days,rows);if(temporal)cards.push(['Confort',temporal]);modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">Méthode Tee · transversal</div><h2><img class="mt-official-star-inline" src="assets/brand-compass-star-transparent.png" alt="" aria-hidden="true">Mes tendances</h2><p class="mt-follow-intro">Corps · Nutrition · Cycle · Confort · Mental. Cette vue relie seulement les journées réellement comparables et privilégie l’ordre temporel lorsqu’il compte.</p><div class="mt-global-trends">${cards.length?cards.map(([cat,x])=>`<div class="mt-global-trend"><small>${esc(cat)}</small><b>${esc(x.title)}</b><p>${esc(x.copy)}</p></div>`).join(''):'<div class="mt-global-trend"><small>PREMIERS REPÈRES</small><b>Ton historique se construit</b><p>Il faut encore quelques journées comparables avant d’afficher une association personnelle suffisamment prudente.</p></div>'}</div><div class="mt-follow-help">Association observée ≠ relation de cause à effet. Les absences de données restent absentes et ne sont jamais transformées en zéro.</div></section>`;};
