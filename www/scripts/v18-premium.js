...
(function(){
  ...
  function mtProtocolSaveError(error){
    ...
    if(message.includes('PROGRESS_NOT_FOUND'))return 'Recharge ce protocole pour initialiser ta progression, puis réessaie.';
    if(message.includes('PROGRESS_AMBIGUOUS'))return 'Ta progression doit être vérifiée. Contacte-nous depuis Assistance.';
    if(message.includes('XP_DATE_INVALID'))return 'Vérifie la date de ton appareil, puis réessaie.';
    if(message.includes('AUTH_REQUIRED')||message.includes('XP_SESSION_CHANGED'))return 'Reconnecte-toi pour confirmer ta progression.';
    if(error?.code==='PGRST202'||message.includes('garden_complete_protocol_action'))
      return 'Le service de progression doit être mis à jour. Ta validation n’a pas été confirmée.';
    return 'Validation non confirmée. Réessaie : tes points ne seront pas comptés deux fois.';
  }
  function mtApplyProtocolXP(protocolId,progress){
    const id=String(protocolId);
    document.querySelectorAll('[data-protocol-xp]').forEach(el=>{
      if(el.dataset.protocolXp===id)el.textContent=String(Number(progress.xp)||0);
    });
    document.querySelectorAll('[data-protocol-level]').forEach(el=>{
      if(el.dataset.protocolLevel===id)el.textContent=String(progress.level_label||mtComputeLevel(progress.xp).label);
    });
    document.querySelectorAll('[data-protocol-streak]').forEach(el=>{
      if(el.dataset.protocolStreak===id)el.textContent=String(Number(progress.streak)||0);
    });
  }
  async function mtSaveProtocolAction(client,user,protocolId,actionKey,contentId=null){
    const result=await mtPromiseTimeout(client.rpc('garden_complete_protocol_action',{
      target_protocol:String(protocolId),action_key:actionKey,
      target_content:contentId===null?null:String(contentId),target_date:todayKey()
    }),12000,null);
    if(!result)throw new Error('XP_SAVE_TIMEOUT');
    if(result.error)throw result.error;
    const data=result.data;
    if(!data?.progress||!['saved','already_done','complete'].includes(data.status)||
       String(data.progress.protocol_id)!==String(protocolId)||String(data.progress.user_id)!==String(user.id))
      throw new Error('XP_SAVE_INVALID');
    const active=await mtGetUser();
    if(!active||active.id!==user.id)throw new Error('XP_SESSION_CHANGED');
    if(window.mtAcceptGardenResult)await window.mtAcceptGardenResult(user.id,data);
    mtApplyProtocolXP(protocolId,data.progress);
    return data;
  }
  ...
  window.mtValidateProtocolToday=async function(protocolId,totalDays){
    ...
    if(!client||!user){if(window.mtToast)mtToast('Connecte-toi pour enregistrer ta journée.','error');return;}
    const key=todayKey(),lockKey=user.id+':'+protocolId+':'+key;
    if(window.__mtValidatingProtocolDay[lockKey])return;
    window.__mtValidatingProtocolDay[lockKey]=true;
    const buttons=Array.from(document.querySelectorAll('.validate-today-btn,.validate-journey-btn'));
    const originals=buttons.map(b=>({b,text:b.textContent,disabled:b.disabled,done:b.classList.contains('done')}));
    let confirmed=false;
    buttons.forEach(b=>{b.disabled=true;b.textContent='Enregistrement…';});
    try{
      ...
      if(result.status==='complete'&&!done.includes(key)){
        ...
        if(window.mtToast)mtToast('Toutes les journées de ce protocole sont déjà validées.');
        return;
      }
      if(!done.includes(key))throw new Error('XP_DAY_NOT_CONFIRMED');
      confirmed=true;
      buttons.forEach(b=>{b.disabled=true;b.classList.add('done');b.textContent='✓ Journée validée';});
      ...
      if(window.mtToast)mtToast(mtProtocolSaveError(error),'error');
    }finally{
      delete window.__mtValidatingProtocolDay[lockKey];
      if(!confirmed)originals.forEach(({b,text,disabled,done})=>{
        b.disabled=disabled;b.textContent=text;b.classList.toggle('done',done);
      });
    }
  };
  ...
  window.mtMarkContentDone=async function(contentId,protocolId,button){
    ...
    try{
      ...
      document.querySelectorAll('[data-content-id]').forEach(el=>{
        if(String(el.dataset.contentId)===id)el.classList.add('is-done');
      });
      const gain=Math.max(0,Number(result.gained)||0);
      if(window.mtToast)mtToast(result.status==='already_done'?'Contenu déjà terminé':('Contenu terminé'+(gain?' · +'+gain+' XP':'')));
      const next=button?.closest('.viewer-actions')?.querySelector('.mt-next-content-btn');
      if(next)next.classList.add('is-highlighted');
      // XP and levels have already been updated from result.progress.
      // No catalogue reload (and no extra signed media requests) per content.
    }catch(error){
  ...
  function renderProgress(protocol, progress){
    ...
    return `<section class="protocol-progress-card reveal visible"><div class="protocol-progress-top"><div><small>Progression privée</small><h2>Jour ${day}/${total}</h2></div><div class="progress-ring-text">${pct}%</div></div><div class="progress-bar"><span style="width:${pct}%"></span></div><div class="protocol-progress-meta"><div><b data-protocol-streak="${safe(protocol.id)}">${Number(progress?.streak||0)}</b><span>Streak</span></div><div><b data-protocol-xp="${safe(protocol.id)}">${Number(progress?.xp||0)}</b><span>XP</span></div><div><b data-protocol-level="${safe(protocol.id)}">${safe(progress?.level_label||protocol.level_label||'Glow')}</b><span>Niveau</span></div></div><button class="validate-today-btn ${validatedToday?'done':''}" onclick="mtValidateProtocolToday('${safe(protocol.id)}',${total})">${validatedToday?'Validé aujourd’hui':'Valider aujourd’hui'}</button></section><section class="timeline-rail">${days}</section>`;
  }
  ...
  const MT_CARNET_TRACKER_META={
    ...
    stress_regulation:{title:'Stress & régulation',description:'Tension, charge mentale, humeur et stratégies de régulation.'},
  ...
  function mtEnsureCarnetTrackerCSS(){
    ...
    const style=document.createElement('style');style.id='mt-carnet-follow-css';style.textContent=`
      ...
      .carnet-follow-empty{padding:18px;border-radius:22px;background:rgba(244,237,226,.68);color:#806f61;font-size:.88rem;line-height:1.5;margin:18px 0}.carnet-follow-more{width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid rgba(177,138,67,.30);border-radius:18px;background:rgba(255,253,248,.72);color:#173f35;padding:13px 15px;margin:-2px 0 18px;font-weight:850}.carnet-follow-more small{display:block;color:#8d7d6e;font-size:10px;font-weight:650;margin-top:2px}.carnet-follow-more>span:last-child{color:#a17d3d;font-family:serif;font-size:22px;transition:transform .18s}.carnet-follow-more.is-open>span:last-child{transform:rotate(90deg)}.carnet-follow-extra{display:none}.carnet-follow-extra.is-open{display:contents}.carnet-trends-link{width:100%;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:12px;border:0;border-top:1px solid rgba(177,138,67,.18);border-bottom:1px solid rgba(177,138,67,.18);background:transparent;color:#173f35;padding:15px 3px;margin:3px 0 16px;text-align:left}.carnet-trends-link-mark{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#f2eadc}.carnet-trends-link-mark{color:#b08a43;font-size:16px}.carnet-trends-link strong{display:block;font-family:var(--font-serif,"Cormorant Garamond",serif);font-size:1.28rem;font-weight:600}.carnet-trends-link small{display:block;color:#8d7d6e;font-size:10px;line-height:1.4;margin-top:2px}.carnet-trends-link-arrow{color:#a17d3d;font-family:serif;font-size:23px}.carnet-follow-add{width:100%;border:1px solid #cfb780;border-radius:999px;background:transparent;color:#173f35;padding:14px 18px;font-weight:900;letter-spacing:.04em}.carnet-healthkit-hint{display:flex;align-items:center;gap:10px;margin-top:13px;padding:4px 5px 2px;color:#7f7368;text-decoration:none}.carnet-healthkit-hint-mark{color:#b08a43;font-size:13px}.carnet-healthkit-hint span:nth-child(2){display:grid;gap:2px;flex:1}.carnet-healthkit-hint strong{font-size:11px;line-height:1.35;color:#315248;font-weight:850}.carnet-healthkit-hint small{font-size:9px;line-height:1.45;color:#988c80}.carnet-healthkit-hint-arrow{font-family:serif;font-size:22px;color:#b08a43}.carnet-follow-manage{border:0;background:transparent;color:#9a7636;font-weight:850;padding:8px 0}
  ...
  function mtCarnetTrackerCardHTML(key,prefs){
    const meta=MT_CARNET_TRACKER_META[key],settings=prefs[key]?.settings||{},summary=settings.latest_summary||(key==='performance_recuperation'&&settings.discipline?`${settings.discipline} · prêt à renseigner`:meta.description);
    return `<article class="carnet-follow-card"><small>${safe(mtTrackerDateLabel(settings.latest_date))}</small><h3>${safe(meta.title)}</h3><p>${safe(summary)}</p><footer><button type="button" onclick="mtOpenCarnetTrackingEntry('${safe(key)}')">Saisir aujourd’hui</button><button type="button" onclick="mtOpenCarnetTrackingHistory('${safe(key)}')">Évolution 7/28 j</button></footer></article>`;
  }
  function mtCarnetTrackerCardsHTML(userId){
    ...
    const visible=active.slice(0,3),extra=active.slice(3);
    return `<div class="carnet-follow-list">${visible.map(key=>mtCarnetTrackerCardHTML(key,prefs)).join('')}${extra.length?`<div class="carnet-follow-extra" id="mtCarnetTrackerExtra">${extra.map(key=>mtCarnetTrackerCardHTML(key,prefs)).join('')}</div>`:''}</div>${extra.length?`<button class="carnet-follow-more" id="mtCarnetTrackerMore" type="button" onclick="mtToggleCarnetTrackerExtras(this)"><span>Voir ${extra.length} autre${extra.length>1?'s':''} suivi${extra.length>1?'s':''}<small>Tous restent actifs et continuent de construire leur historique.</small></span><span>›</span></button>`:''}`;
  }
  window.mtToggleCarnetTrackerExtras=function(button){const extra=document.getElementById('mtCarnetTrackerExtra');if(!extra)return;const open=!extra.classList.contains('is-open');extra.classList.toggle('is-open',open);button?.classList.toggle('is-open',open);const count=extra.querySelectorAll('.carnet-follow-card').length;const text=button?.querySelector('span:first-child');if(text)text.innerHTML=open?`Masquer les autres suivis<small>Les ${count} repères restent accessibles dans Gérer.</small>`:`Voir ${count} autre${count>1?'s':''} suivi${count>1?'s':''}<small>Tous restent actifs et continuent de construire leur historique.</small>`;};
  function mtCarnetTrackingShelfHTML(userId){
    mtEnsureCarnetTrackerCSS();
    return `<section id="mtCarnetTrackingShelf" class="biblio-smart-shelf carnet-follow-shelf reveal mt-premium-arrival"><div class="biblio-shelf-kicker">Carnet personnel</div><div class="carnet-follow-head"><div><h2>Mes suivis</h2><p>Retrouve immédiatement les repères que tu as choisis.</p></div><button class="carnet-follow-manage" type="button" onclick="mtOpenCarnetAddTracking()">Gérer</button></div><div id="mtCarnetTrackerCards" data-user-id="${safe(userId)}">${mtCarnetTrackerCardsHTML(userId)}</div><button class="carnet-trends-link" type="button" onclick="mtOpenCarnetGlobalTrends()"><span class="carnet-trends-link-mark">✷</span><span><strong>Mes tendances</strong><small>Relier mes repères sur les 28 derniers jours, seulement quand ils sont comparables.</small></span><span class="carnet-trends-link-arrow">›</span></button><button class="carnet-follow-add" type="button" onclick="mtOpenCarnetAddTracking()">+ Ajouter un suivi</button><a class="carnet-healthkit-hint" href="dashboard.html#apple-sante"><span class="carnet-healthkit-hint-mark">✷</span><span><strong>Compléter automatiquement certains repères avec Apple Santé</strong><small>Sommeil, marche, récupération, cycle et autres repères compatibles · connexion facultative</small></span><span class="carnet-healthkit-hint-arrow">›</span></a></section>`;
  }
  ...
  window.mtEnsureAdvancedTrackers = async function(){
    ...
    window.__MT_ADVANCED_TRACKERS_LOADING__ = new Promise((resolve,reject)=>{
      ...
      script.onload=()=>{
        ...
        const analytics=document.createElement('script');analytics.src='scripts/v419-analytics.js?v=v419-full-analytics-r3';analytics.async=true;analytics.onload=finish;analytics.onerror=finish;document.head.appendChild(analytics);
      };
      script.onerror=()=>{
  ...
  window.mtOpenCarnetGlobalTrends = async function(){
    try{await window.mtEnsureAdvancedTrackers();return window.mtAdvancedTrackerGlobalTrends?.(28);}catch(e){return null;}
  };
