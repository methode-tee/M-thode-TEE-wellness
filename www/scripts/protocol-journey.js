...
async function mtLoadProtocolCurve(user,progress,total){
  const client=initSupabase&&initSupabase(),start=mtProtocolStartDate(progress),end=mtAddProtocolDays(start,total-1);if(!client||!user)return {start,days:[]};
  try{const {data}=await client.from('daily_activity').select('activity_date,tee_balance_snapshot').eq('user_id',user.id).gte('activity_date',start).lte('activity_date',end).order('activity_date');return {start,days:Array.isArray(data)?data:[]};}catch(_){return {start,days:[]};}
}
function mtProtocolCurveSVG(series,key,color,total){
  const points=series.map((item,index)=>({index,value:Number(item?.[key])})).filter(item=>Number.isFinite(item.value));if(points.length<2)return '';
  const x=index=>14+index*(292/Math.max(1,total-1)),y=value=>106-Math.max(0,Math.min(100,value))*.88;let path='',previous=-2;
  points.forEach(point=>{path+=`${point.index===previous+1&&path?'L':'M'}${x(point.index).toFixed(1)} ${y(point.value).toFixed(1)} `;previous=point.index;});
  return `<path d="${path}" fill="none" stroke="${color}" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>${points.map(point=>`<circle cx="${x(point.index)}" cy="${y(point.value)}" r="3.2" fill="${color}"><title>Jour ${point.index+1} · ${Math.round(point.value)} %</title></circle>`).join('')}`;
}
function mtRenderProtocolCurve(curve,progress,total){
  const byDate=new Map((curve.days||[]).map(row=>[row.activity_date,row.tee_balance_snapshot||{}])),series=Array.from({length:total},(_,index)=>byDate.get(mtAddProtocolDays(curve.start,index))||{}),count=series.filter(item=>['vitality','inner','regularity'].some(key=>Number.isFinite(Number(item[key])))).length;
  const svg=count>=2?`<svg viewBox="0 0 320 122" role="img" aria-label="Évolution de Mon Équilibre pendant ce protocole" style="width:100%;height:auto;display:block"><line x1="14" y1="106" x2="306" y2="106" stroke="#e4dacb"/><line x1="14" y1="62" x2="306" y2="62" stroke="#eee6da" stroke-dasharray="4 5"/>${mtProtocolCurveSVG(series,'vitality','#17483d',total)}${mtProtocolCurveSVG(series,'inner','#b48a38',total)}${mtProtocolCurveSVG(series,'regularity','#7e8d84',total)}</svg>`:`<div style="padding:22px;border-radius:20px;background:#f4eee4;color:#786b5f;line-height:1.5">La courbe se dessinera après deux journées renseignées pendant ce protocole.</div>`;
  return `<section class="journey-section"><div class="journey-section-kicker">Mon évolution pendant ce protocole</div><div class="journey-section-title">Du jour 1 au jour ${total}</div><p class="journey-section-sub">Chaque journée enregistrée nourrit cette lecture. Les jours sans donnée restent vides et ne deviennent jamais zéro.</p>${svg}<div style="display:flex;gap:13px;flex-wrap:wrap;margin-top:10px;color:#75695d;font-size:11px"><span>● <b style="color:#17483d">Vitalité</b></span><span>● <b style="color:#b48a38">Équilibre intérieur</b></span><span>● <b style="color:#7e8d84">Régularité</b></span><span>${count}/${total} jours documentés</span></div></section>`;
}
window.renderProtocolJourney=async function(){
    ...
    const progress=await getProgress(protocol); const total=durationDays(protocol); const s=score(progress,total); const intention=INTENTIONS[(Number(progress.current_day||1)-1)%INTENTIONS.length]; const [contents,protocolCurve]=await Promise.all([getContents(protocol, progress, admin),mtLoadProtocolCurve(user,progress,total)]); const done=mtNormalizeCompletedDays(progress.completed_days); const validated=done.includes(todayKey());
    ...
    document.querySelector('.journey-section--days')?.insertAdjacentHTML('beforebegin',mtRenderProtocolCurve(protocolCurve,progress,total));
