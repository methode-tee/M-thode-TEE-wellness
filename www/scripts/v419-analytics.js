...
(function(){
  ...
  function coverageStatus(coverage,key){return coverage?.[key]?.status||'undocumented';}
  function microCard(key,micros,sources,coverage){
    const value=micros?.[key],details=sourceDetails(sources?.[key],key),status=coverageStatus(coverage,key),isKnown=status==='complete'&&known(value);
    const copy=isKnown?`${esc(rounded(value))} ${esc(units(key))} calculés`:status==='partial'?'Données partielles':'Non documenté sur cette journée';
    return `<span style="display:block;padding:10px 11px;border-radius:14px;background:#fffdf8;border:1px solid #e6dccd"><b style="display:block;color:#173b31">${esc(labels[key]||key)}</b><small>${copy}</small>${details.length?`<small style="display:block;margin-top:4px;color:#887a6d">Sources documentées : ${esc(details.slice(0,3).join(' · '))}</small>`:`<small style="display:block;margin-top:4px;color:#887a6d">Aucune source calculable renseignée</small>`}</span>`;
  }
  function adviceHTML(settings,micros,sources,microCoverage){
    const profile=String(settings?.diet_pattern||''),out=[];
    if(coverageStatus(microCoverage,'iron_mg')==='complete'&&known(micros?.iron_mg)&&sourceNames(sources?.iron_mg).length){out.push('<b>Fer végétal</b><p>Une ou plusieurs sources de fer sont renseignées. Associer une source de vitamine C au même repas peut être un repère alimentaire intéressant. Cette lecture ne mesure pas tes réserves en fer.</p>');}
    const b12Complete=coverageStatus(microCoverage,'vitamin_b12_ug')==='complete',b12Known=b12Complete&&known(micros?.vitamin_b12_ug)&&Number(micros.vitamin_b12_ug)>0,b12Supplement=settings?.supplement_b12==='Oui';
    if(/^Végane$/i.test(profile)&&b12Complete&&!b12Known&&!b12Supplement){out.push('<b>Vitamine B12</b><p>Sur les aliments entièrement documentés de cette journée, aucune source calculée de B12 ni supplémentation n’est renseignée. La B12 mérite une attention spécifique dans une alimentation végétalienne ; ce repère ne permet pas d’évaluer ton statut biologique et ne prescrit aucune dose.</p>');}
    const omegaComplete=coverageStatus(microCoverage,'omega3_g')==='complete';
    if(omegaComplete&&known(micros?.omega3_g)&&Number(micros.omega3_g)<=0){out.push('<b>À explorer · oméga-3</b><p>Les graines de chia ou de lin et les noix peuvent apporter de l’ALA. Il s’agit d’une suggestion alimentaire séparée de ce que tu as réellement consommé ; l’app n’évalue pas ton statut biologique en oméga-3.</p>');}
  ...
  window.mtV419EnhanceTrackerForm=async function(key,date,settings={}){
    ...
    try{
      ...
      const summary=data||{},micros=summary.micronutrients&&typeof summary.micronutrients==='object'?summary.micronutrients:{},microCoverage=summary.micronutrient_coverage&&typeof summary.micronutrient_coverage==='object'?summary.micronutrient_coverage:{},sources=summary.micronutrient_sources&&typeof summary.micronutrient_sources==='object'?summary.micronutrient_sources:{},sourceFoods=Array.isArray(summary.source_foods)?summary.source_foods:[],quality=qualityModel(summary);
      hiddenIfKnown(form,'meal_count',summary.meal_count);hiddenIfKnown(form,'calculated_meals',summary.calculated_meals);hiddenIfKnown(form,'item_count',summary.item_count);hiddenIfKnown(form,'quantified_items',summary.quantified_items);hiddenIfKnown(form,'calculable_items',summary.calculable_items);hiddenIfKnown(form,'protein_g',summary.protein_g);hiddenIfKnown(form,'fiber_g',summary.fiber_g);hiddenIfKnown(form,'carbs_g',summary.carbs_g);hiddenIfKnown(form,'fat_g',summary.fat_g);hiddenIfKnown(form,'micronutrient_coverage_count',summary.micronutrient_coverage_count);hiddenIfKnown(form,'micronutrient_source_count',summary.micronutrient_source_count);hidden(form,'_micronutrients',micros);hidden(form,'_micronutrient_coverage',microCoverage);hidden(form,'_micronutrient_sources',sources);hidden(form,'_nutrition_source_foods',sourceFoods);hidden(form,'_nutrition_data_quality',quality.level);
      ...
      const microKeys=observed.filter(k=>labels[k]),microCards=microKeys.map(k=>microCard(k,micros,sources,microCoverage)).join('');
      ...
      host.innerHTML=`<small>Carnet alimentaire · ${esc(date)}</small><b>${esc(quality.title)}</b><p>${esc(qualityLine)}. ${esc(quality.copy)}</p>${addMeal}${quality.level!=='none'?`<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:10px">${macroCards}</div>`:''}${microCards?`<div style="margin-top:12px"><small style="display:block;margin-bottom:6px">Micronutriments observés</small><div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px">${microCards}</div></div>`:''}${adviceHTML(settings,micros,sources,microCoverage)}<p style="margin-top:10px">Cette lecture repose uniquement sur les aliments et quantités renseignés. Elle ne permet pas de diagnostiquer une carence, d’évaluer un taux sanguin ou de prescrire une supplémentation.</p>`;
