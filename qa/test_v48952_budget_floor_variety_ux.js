...
let code=rawCode;
code=code.replace("document.addEventListener('DOMContentLoaded',init);","globalThis.__TEE_V48952_TEST={optimizeWeekV489,candidateTrueVarietyKeys,canonicalProteinFamilyV48951,canonicalStarchFamilyV48951};");
...
const {optimizeWeekV489,candidateTrueVarietyKeys,canonicalProteinFamilyV48951,canonicalStarchFamilyV48951}=ctx.__TEE_V48952_TEST;
...
const plan30=optimizeWeekV489({...base,candidates:candidates30,budget:30,userId:'qa-v48952-30'});
...
assert(code.includes('orientPlannerResult(result,{focus:true});'),'orientation automatique robuste absente');
...
assert(html.includes('v48952-budget-floor-dur-variete-ux-preparation-r1'),'cache bust V489.5.2 absent');
...
assert(rawCode.includes('budgetFloorExpansionPoolV48952'),'pool de sauvetage budget large absent');
assert(rawCode.includes('state.cost+maxPossible<budget*Number(policy.hardFloorRatio||0)'),'pruning de faisabilité budget absent');
assert(css.includes('mt-next-kitchen-loader'),'animation de préparation culinaire absente');
...
console.log(JSON.stringify({status:'ok_v48952',cost_45:Number(plan.cost.toFixed(2)),floor_45:35.10,cost_30:Number(plan30.cost.toFixed(2)),floor_30:23.40,chicken_meals:chicken,variety_relaxation:plan.varietyRelaxationUsed,true_variety_counts:counts},null,2));
