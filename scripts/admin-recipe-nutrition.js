(function(){
  ...
  async function load(recipeId){
    ...
    try{const {data,error}=await client.rpc('mt_get_recipe_nutrition_admin',{p_recipe_id:recipeId});if(error)throw error;const profile=data?.profile||{};items=Array.isArray(data?.items)?data.items.map(x=>({...x})):[];$('mtRecipeNutritionServings').value=Number(profile.servings)||1;$('mtRecipeNutritionServingLabel').value=profile.serving_label||'1 portion';render();if(data?.snapshot&&snap){snap.hidden=false;snap.textContent=`Calcul actuel · ${snapshotText(data.snapshot) || 'repères partiels'}`;}$('mtRecipeNutritionStatus').textContent=items.length?'Nutrition structurée chargée. Tu peux modifier les grammes puis recalculer.':'Cette recette n’a pas encore de calcul structuré.';}catch(e){console.warn('recipe nutrition admin load',e);$('mtRecipeNutritionStatus').textContent='Calcul structuré indisponible tant que la migration V411.2 n’est pas installée.';}
  }
  async function save(){
  ...
  async function clear(){
    ...
    try{const {error}=await sb().rpc('mt_admin_save_recipe_nutrition',{p_recipe_id:recipeId,p_servings:Number($('mtRecipeNutritionServings').value)||1,p_serving_label:$('mtRecipeNutritionServingLabel').value||'1 portion',p_items:[]});if(error)throw error;items=[];render();$('mtRecipeNutritionSnapshot').hidden=true;$('mtRecipeNutritionStatus').textContent='Calcul structuré retiré.';}catch(e){alert(e.message||'Impossible de retirer le calcul.');}
  }
  function bind(){
  ...
  function hook(){
    ...
    const baseReset=window.resetRecipeForm;if(typeof baseReset==='function'&&!baseReset.__mtNutrition){const fn=function(){const r=baseReset.apply(this,arguments);items=[];render();if($('mtRecipeNutritionServings'))$('mtRecipeNutritionServings').value=1;if($('mtRecipeNutritionServingLabel'))$('mtRecipeNutritionServingLabel').value='1 portion';if($('mtRecipeNutritionSnapshot'))$('mtRecipeNutritionSnapshot').hidden=true;if($('mtRecipeNutritionStatus'))$('mtRecipeNutritionStatus').textContent='Enregistre d’abord la recette, puis ouvre « Modifier » pour renseigner ses ingrédients structurés.';return r;};fn.__mtNutrition=true;window.resetRecipeForm=fn;}
  }
