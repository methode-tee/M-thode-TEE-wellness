...
function mtProtocolLens(protocol){
  ...
  const explicit=[
    ...
    [/prise de masse|masse saine/,{kind:'mass_gain',keys:['performance_recuperation','evolution_corporelle','nutrition_vegetale'],title:'Construction & récupération',subtitle:'Pratique, récupération, protéines et évolution corporelle sont suivies sans transformer le poids en note.',health:['activity','body']}],
...
async function mtLoadProtocolModel(user,protocol,progress,total,referencePromise=null){
  ...
  else if(lens.kind==='recomposition'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolCount(rp,'Entraînements',v=>mtProtocolNum(v.duration)>0||Boolean(v.session),'Journées avec entraînement renseigné.','Suivi activité'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolNumeric(body,'Poids · plage',v=>mtProtocolNum(v.weight),'kg','range',sourceLabel),mtProtocolNumeric(nutrition,'Protéines',v=>mtProtocolNum(v.protein_g),'g','average','Carnet alimentaire'));}
  else if(lens.kind==='mass_gain'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolNumeric(rp,'Temps d’entraînement',v=>mtProtocolNum(v.duration),'min','average','Suivi activité'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolNumeric(nutrition,'Protéines documentées',v=>mtProtocolNum(v.protein_g),'g','average','Carnet alimentaire'),mtProtocolNumeric(body,'Poids · plage',v=>mtProtocolNum(v.weight),'kg','range',sourceLabel));}
  ...
  else if(lens.kind==='silhouette'){const body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale'),rp=rowsFor('performance_recuperation');metrics.push(mtProtocolNumeric(body,'Poids · plage',v=>mtProtocolNum(v.weight),'kg','range',sourceLabel),mtProtocolNumeric(body,'Tour de taille · plage',v=>mtProtocolNum(v.waist),'cm','range',sourceLabel),mtProtocolNumeric(rp,'Mouvement / pratique',v=>mtProtocolNum(v.duration),'min','average','Suivi activité'),mtProtocolNumeric(nutrition,'Protéines documentées',v=>mtProtocolNum(v.protein_g),'g','average','Carnet alimentaire'));}
