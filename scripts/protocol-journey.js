/* V412.1 — protocoles connectés au Profil + Carnet, protéines documentées et validation robuste. */
/* =========================================================
...
function mtNormalizeCompletedDays(value){
  ...
  function renderContent(c,pid){
    ...
    return `<article class="journey-content-card" data-content-id="${safe(c.id||'')}"${phytoIds?` data-mt-phyto-ids="${safe(phytoIds)}"`:''} onclick="openPremiumContent('${enc}','${safe(pid)}')">
...
function mtProtocolLens(protocol){
  const text=mtProtocolNorm(`${protocol?.title||''} ${protocol?.slug||''}`),slug=String(protocol?.slug||'');
  // V446 : « Premiers Pas » est un parcours d'entrée transversal, pas un protocole de marche.
  // On le traite AVANT le filet de sécurité /pas/ afin que son nom ne déclenche jamais
  // artificiellement les cartes « Mouvement & récupération ».
  if(slug==='premiers-pas-la-methode-tee'||/premiers? pas.*methode tee/.test(text))return {kind:'first_steps',keys:[],title:'Tes premiers repères',subtitle:'Ce parcours découvre ta manière d’utiliser Méthode Tee : validations, ressentis et données réellement renseignées restent reliés sans imposer un thème sportif.',health:[]};
  // V421 : configurations analytiques explicites protocole par protocole.
  // Les familles ci-dessous ne sont qu'un filet de sécurité pour les futurs protocoles.
  const explicit=[
    [/pilates/,{kind:'pilates',keys:['performance_recuperation','pas_marche'],title:'Ta pratique Pilates',subtitle:'Temps pratiqué, séances, aisance et récupération : une lecture dédiée au Pilates.',health:['activity']}],
    [/recomposition|definition/,{kind:'recomposition',keys:['performance_recuperation','evolution_corporelle','nutrition_vegetale','pas_marche'],title:'Ta recomposition pendant le protocole',subtitle:'Entraînement, récupération, repères corporels neutres et protéines restent séparés.',health:['activity','body']}],
    [/force.*construction|construction.*force|prise de masse|masse saine/,{kind:'mass_gain',keys:['performance_recuperation','evolution_corporelle','nutrition_vegetale','pas_marche'],title:'Construction & récupération',subtitle:'Pratique, récupération, protéines documentées et évolution corporelle sont reliées au Carnet, au Profil et aux suivis réellement renseignés.',health:['activity','body']}],
    [/energie.*vitalite|vitalite.*energie/,{kind:'vitality',keys:['performance_recuperation','pas_marche','sommeil_profond','nutrition_vegetale','stress_regulation'],title:'Énergie & vitalité observées',subtitle:'Mouvement, sommeil, énergie ressentie et apports documentés restent reliés sans transformer la journée en score.',health:['activity','sleep']}],
    [/souplesse.*mobilite|mobilite.*souplesse/,{kind:'mobility',keys:['performance_recuperation','pas_marche'],title:'Souplesse, mobilité & récupération',subtitle:'Pratique, mobilité ressentie, récupération et mouvement sont reliés aux suivis réellement renseignés.',health:['activity']}],
    [/malbouffe.*exces|exces.*malbouffe/,{kind:'food_balance',keys:['nutrition_vegetale','equilibre_alimentaire','digestion'],title:'Repères alimentaires pendant le protocole',subtitle:'Fibres, protéines, sucres documentés, satiété et digestion sont lus depuis le Carnet lorsqu’ils existent.',health:[]}],
    [/3 jours.*retrouver.*rythme|retrouver.*rythme/,{kind:'rhythm',keys:['performance_recuperation','pas_marche','sommeil_profond','equilibre_alimentaire'],title:'Ton rythme observé',subtitle:'Validations, mouvement, sommeil et ressentis réellement renseignés restent reliés pendant ces trois jours.',health:['activity','sleep']}],
    [/confort dentaire/,{kind:'dental',keys:['boissons','sommeil_profond'],title:'Confort & repères du quotidien',subtitle:'Validations, hydratation renseignée, ressentis et récupération restent reliés sans interprétation médicale.',health:['sleep']}],
    [/crampes?.*recuper|recuper.*crampes?/,{kind:'cramps',keys:['performance_recuperation','pas_marche'],title:'Récupération & confort musculaire',subtitle:'Récupération, gêne et activité sont décrites sans diagnostic.',health:['activity']}],
    [/silhouette.*equilibre/,{kind:'silhouette',keys:['evolution_corporelle','nutrition_vegetale','performance_recuperation','pas_marche'],title:'Silhouette & habitudes observées',subtitle:'Mensurations neutres, mouvement et apports documentés restent des repères descriptifs.',health:['activity','body']}],
    [/ventre plat/,{kind:'flat_belly',keys:['digestion','evolution_corporelle'],title:'Confort abdominal & repères neutres',subtitle:'Confort digestif et tour de taille sont observés sans promettre de résultat corporel.',health:['body']}],
    [/sommeil profond/,{kind:'sleep',keys:['sommeil_profond'],title:'Tes nuits pendant ce protocole',subtitle:'Durée, qualité et réveil restent séparés ; Apple Santé complète seulement les mesures disponibles.',health:['sleep']}],
    [/stress.*cortisol/,{kind:'stress',keys:['sommeil_profond'],title:'Présence & apaisement',subtitle:'Rituels, ressentis et sommeil documenté : aucune jauge ne note ton niveau de réussite.',health:['sleep']}],
    [/anxiete|apaisement/,{kind:'stress',keys:['sommeil_profond'],title:'Repères d’apaisement',subtitle:'Présence, ressentis et sommeil sont observés avec prudence.',health:['sleep']}],
    [/cycle.*hormon/,{kind:'cycle',keys:['cycle'],title:'Tes repères de cycle',subtitle:'Énergie, sommeil et symptômes sont observés sans jauge « bonne / mauvaise ».',health:[]}],
    [/periode menstruelle|menstru/,{kind:'cycle',keys:['cycle'],title:'Tes repères pendant les règles',subtitle:'Énergie, douleurs et sommeil sont décrits sans juger la journée.',health:[]}],
    [/maux de ventre/,{kind:'digestion',keys:['digestion'],title:'Ton confort abdominal',subtitle:'Douleurs, ballonnements et confort sont suivis uniquement lorsqu’ils sont renseignés.',health:[]}],
    [/reset ventre|digest|ballonn|transit/,{kind:'digestion',keys:['digestion'],title:'Ton confort digestif',subtitle:'Les balises suivent uniquement les journées réellement renseignées.',health:[]}],
    [/stop sucre|sucre/,{kind:'sugar',keys:['reduction_sucre','nutrition_vegetale'],title:'Ton rapport au sucre',subtitle:'Envies, alternatives et journées documentées, sans logique de réussite ou d’échec.',health:[]}],
    [/jeune|intermittent/,{kind:'fast',keys:['jeune_intermit'],title:'Ton rythme de jeûne',subtitle:'La durée n’est jamais transformée en note ; le confort reste prioritaire.',health:[]}],
    [/aigreur|reflux|estomac/,{kind:'reflux',keys:['reflux'],title:'Tes repères de reflux',subtitle:'Fréquence, intensité et contexte sont décrits sans attribuer automatiquement une cause.',health:[]}],
    [/perimenopause|menopause/,{kind:'perimenopause',keys:['perimenopause','fringales_envies','sommeil_profond','stress_regulation'],title:'Tes repères de transition hormonale',subtitle:'Symptômes, sommeil, énergie, stress et envies restent reliés sans attribuer automatiquement une cause.',health:['sleep']}],
    [/peau|acne|imperfection/,{kind:'skin',keys:['peau'],title:'Ta peau pendant le protocole',subtitle:'Confort, sensibilité et contexte sont comparés sans diagnostic.',health:[]}],
    [/anemie|\bfer\b|micronutr/,{kind:'nutrition',keys:['nutrition_vegetale'],title:'Tes apports documentés',subtitle:'Uniquement les aliments quantifiés et nutriments réellement disponibles dans le Carnet.',health:[]}],
    [/alcool.*equilibre|reset alcool/,{kind:'alcohol',keys:['sommeil_profond'],title:'Rythme & récupération',subtitle:'Présence dans le protocole, ressentis et sommeil documenté sont suivis sans moraliser.',health:['sleep']}],
    [/maux de gorge|gorge/,{kind:'gentle',keys:['sommeil_profond'],title:'Confort & récupération',subtitle:'Le protocole conserve tes validations et tes ressentis ; le sommeil peut compléter la lecture lorsqu’il est disponible.',health:['sleep']}]
  ];
  for(const [rx,config] of explicit) if(rx.test(text)) return config;
  ...
  if(/cycle|hormon|regles/.test(text))return {kind:'cycle',keys:['cycle'],title:'Tes repères de cycle',subtitle:'Énergie, sommeil et symptômes sont observés sans jauge « bonne / mauvaise ».',health:[]};
  if(/silhouette|poids/.test(text))return {kind:'body',keys:['evolution_corporelle'],title:'Tes repères corporels',subtitle:'Plages et ressentis neutres : aucun poids ni mensuration n’est transformé en score.',health:['body']};
  if(/sport|endurance|mouvement|marche|pas|muscl|recuper/.test(text))return {kind:'activity',keys:['performance_recuperation','pas_marche'],title:'Mouvement & récupération',subtitle:'Le suivi combine ressenti personnel et données Apple Santé quand elles sont disponibles.',health:['activity']};
  return {kind:'generic',keys:[],title:'Ton empreinte dans le protocole',subtitle:'Une lecture du parcours lui-même, distincte des jauges générales du Carnet.',health:[]};
}
function mtProtocolPublicSource(source){
  const raw=String(source||'').trim();if(!raw)return 'Méthode Tee';
  const key=raw.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'');
  const map={beverages:'Boissons',boissons:'Boissons',daily_activity:'Activité quotidienne',activity:'Activité',healthkit:'Apple Santé',food_meals:'Alimentation',nutrition:'Alimentation',micronutrition:'Micronutrition',hydration:'Hydratation',journal_entries:'Journal & ressentis',user_tracker_entries:'Suivis personnels',protocol:'Protocole',protocols:'Protocoles'};
  if(map[key])return map[key];
  if(/[_./]|\b(sql|rpc|json|uuid|table|entry|entries|source|core|signal|signals)\b/i.test(raw))return 'Méthode Tee';
  return raw;
}
...
function mtProtocolProteinMetric(rows,label='Protéines documentées'){
  const series=(rows||[]).map(row=>{const v=row.values||{},complete=mtProtocolNum(v.protein_g),documented=mtProtocolNum(v.protein_documented_g),coverage=mtProtocolNum(v.protein_coverage_pct),preferDocumented=documented!==null&&documented>0&&(complete===null||complete<=0);return {date:row.entry_date,value:preferDocumented?documented:(complete??documented),partial:preferDocumented||(complete===null&&documented!==null),coverage};}).filter(x=>x.value!==null);
  if(!series.length)return mtProtocolMetric(label,'—','Pas encore documenté.',0,'Carnet alimentaire');
  const avg=mtProtocolAverage(series.map(x=>x.value)),partial=series.filter(x=>x.partial||(x.coverage!==null&&x.coverage<100)).length;
  const detail=partial?`Moyenne des protéines réellement documentées sur ${series.length} journée${series.length>1?'s':''}. ${partial} journée${partial>1?'s':''} reste${partial>1?'nt':''} partielle${partial>1?'s':''} : un aliment non documenté n’est jamais compté comme zéro.`:`Moyenne sur ${series.length} journée${series.length>1?'s':''} entièrement documentée${series.length>1?'s':''}.`;
  return mtProtocolMetric(label,mtProtocolFormat(avg,'g',1),detail,new Set(series.map(x=>x.date)).size,'Carnet alimentaire');
}
function mtProtocolBodyWeightMetric(rows,referenceComparison,label='Poids · plage',source='Méthode Tee'){
  const dated=mtProtocolNumeric(rows,label,v=>mtProtocolNum(v.weight),'kg','range',source);if(dated.count)return dated;
  const weight=mtProtocolNum(referenceComparison?.profile_reference?.weight_kg);if(weight===null)return dated;
  return {...mtProtocolMetric('Poids · repère',mtProtocolFormat(weight,'kg',1),'Poids actuel renseigné dans Mon profil. Il sert de repère connecté, pas d’historique de pesées.',0,'Mon profil'),evidence_mode:'profile'};
}
function mtProtocolMovementMetric(practiceRows,walkRows,source='Méthode Tee'){
  const practice=mtProtocolNumeric(practiceRows,'Mouvement / pratique',v=>mtProtocolNum(v.duration),'min','average','Suivi activité');if(practice.count)return practice;
  return mtProtocolNumeric(walkRows,'Mouvement / pas',v=>mtProtocolNum(v.steps),'pas','average',source);
}
...
async function mtLoadProtocolModel(user,protocol,progress,total,referencePromise=null){
  ...
  let referenceComparison=null;try{referenceComparison=referencePromise?await referencePromise:null;}catch(_){referenceComparison=null;}
  ...
  const healthCats=Array.isArray(lens.health)?lens.health:(lens.health?[lens.health]:[]);
  if(healthCats.includes('activity')&&window.mtHealthKitReadActivityHistory){try{const hk=await window.mtHealthKitReadActivityHistory(start,(end<todayKey()?end:todayKey()),false);(hk?.days||[]).forEach(day=>{if(!day?.date||!day.hasData)return;healthDays.push(day);rows.push({tracker_key:'pas_marche',entry_date:day.date,values:{steps:day.steps,distance_km:day.distanceKm,walking_minutes:day.walkingMinutes,step_length_cm:day.stepLengthCm,walking_speed_kmh:day.walkingSpeedKmh,active_energy_kcal:day.activeEnergyKcal,_healthkit_source:'Apple Santé'}});});if(healthDays.length)sources.add('Apple Santé');}catch(_){}}
  if((healthCats.includes('sleep')||healthCats.includes('body'))&&window.mtHealthKitReadSummary&&todayKey()>=start&&todayKey()<=end){try{const hk=await window.mtHealthKitReadSummary(todayKey(),{categories:healthCats.filter(x=>x==='sleep'||x==='body')});if(healthCats.includes('sleep')&&hk?.sleep?.hasData){const x=hk.sleep;rows.push({tracker_key:'sommeil_profond',entry_date:todayKey(),values:{_sleep_hours:mtProtocolNum(x.durationMinutes)!==null?Number(x.durationMinutes)/60:null,_healthkit_sleep_deep_minutes:x.deepMinutes,_healthkit_sleep_rem_minutes:x.remMinutes,awakenings:x.awakenings,_healthkit_source:'Apple Santé'}});sources.add('Apple Santé');}if(healthCats.includes('body')&&hk?.body){const b=hk.body,v={_healthkit_source:'Apple Santé'},sameDay=item=>{if(!item?.date)return false;const d=new Date(item.date);return !Number.isNaN(d.getTime())&&d.toLocaleDateString('sv-SE')===todayKey();};if(sameDay(b.weightKg))v.weight=b.weightKg.value;if(sameDay(b.waistCm))v.waist=b.waistCm.value;if(sameDay(b.bodyFatPercentage))v.body_fat=b.bodyFatPercentage.value;if(sameDay(b.leanBodyMassKg))v.lean_body_mass=b.leanBodyMassKg.value;if(Object.keys(v).length>1){rows.push({tracker_key:'evolution_corporelle',entry_date:todayKey(),values:v});sources.add('Apple Santé');}}}catch(_){}}
  // V446 : le protocole reçoit aussi les faits compacts déjà construits côté Supabase.
  // Ainsi un repas scanné, une boisson, un poids, un sommeil ou une activité peut nourrir
  // le protocole en cours sans obliger l'utilisateur à ajouter le tracker correspondant.
  // Maximum 60 journées compactes, uniquement lors de l'ouverture du protocole.
  const compactDays=Array.isArray(referenceComparison?.during_days)?referenceComparison.during_days:[];
  const hasAny=v=>Object.values(v||{}).some(x=>x!==null&&x!==undefined&&x!=='');
  compactDays.forEach(day=>{
    ...
    add('nutrition_vegetale',{protein_g:c.protein_g,protein_documented_g:c.protein_documented_g,protein_coverage_pct:c.protein_coverage_pct,fiber_g:c.fiber_g,fat_g:c.fat_g,carbs_g:c.carbs_g,salt_g:c.salt_g,sugars_g:c.sugars_g,saturated_fat_g:c.saturated_fat_g,sodium_g:c.sodium_g,omega3_g:c.omega3_g,omega6_g:c.omega6_g,micronutrient_coverage_count:c.micronutrient_coverage_count,digestion:c.digestion});
    ...
    add('sommeil_profond',{_sleep_hours:c.sleep_hours,quality:c.sleep_quality});
    add('pas_marche',{steps:c.steps,active_energy_kcal:c.active_energy_kcal});
    add('performance_recuperation',{recovery:c.recovery,active_energy_kcal:c.active_energy_kcal});
    add('evolution_corporelle',{weight:c.weight_kg,waist:c.waist_cm});
    add('boissons',{count:c.beverage_count,hydration_liters:c.beverage_hydration_liters,energy_after:c.beverage_energy,digestion_after:c.beverage_digestion,caffeinated_entries:c.beverage_caffeine_count});
  });
  if(compactDays.length)sources.add('Carnet & repères');
  ...
  else if(lens.kind==='sugar'){const r=rowsFor('reduction_sucre'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolNumeric(r,'Intensité des envies',v=>mtProtocolNum(v.craving),'/10'),mtProtocolCount(r,'Jours sans sucre ajouté',v=>v.no_added_sugar==='Oui','Uniquement quand ce repère a été explicitement renseigné.'),mtProtocolCount(r,'Alternatives utiles',v=>v.alternative_help==='Oui'||v.alternative_help==='Un peu','Journées où une alternative a été jugée utile.'),mtProtocolNumeric(nutrition,'Sucres documentés',v=>mtProtocolNum(v.sugars_g),'g','average','Carnet alimentaire'));}
  else if(lens.kind==='fast'){const r=rowsFor('jeune_intermit'),bev=rowsFor('boissons');metrics.push(mtProtocolNumeric(r,'Durée du jeûne',mtProtocolFastHours,'h'),mtProtocolNumeric(r,'Énergie',v=>mtProtocolNum(v.energy),'/10'),mtProtocolNumeric(r,'Confort après rupture',v=>mtProtocolNum(v.break_quality),'/10'),mtProtocolNumeric(bev,'Hydratation renseignée',v=>mtProtocolNum(v.hydration_liters),'L','average','Carnet boissons'));}
  ...
  else if(lens.kind==='perimenopause'){const r=rowsFor('perimenopause'),cravings=rowsFor('fringales_envies');metrics.push(mtProtocolNumeric(r,'Sommeil',v=>mtProtocolNum(v.sleep),'/10'),mtProtocolNumeric(r,'Énergie',v=>mtProtocolNum(v.energy),'/10'),mtProtocolCount(r,'Jours avec bouffées',v=>v.hot_flashes&&!/^Aucune/i.test(String(v.hot_flashes||'')),'Journées où des bouffées de chaleur ont été signalées.'),mtProtocolNumeric(cravings,'Fringales & envies',v=>mtProtocolNum(v.urge_intensity),'/5','average','Suivi Fringales & envies'));}
  ...
  else if(lens.kind==='nutrition'){const r=rowsFor('nutrition_vegetale');metrics.push(mtProtocolProteinMetric(r,'Protéines documentées'),mtProtocolNumeric(r,'Fibres calculées',v=>mtProtocolNum(v.fiber_g),'g'),mtProtocolNumeric(r,'Micronutriments documentés',v=>mtProtocolNum(v.micronutrient_coverage_count),'','average','Carnet alimentaire'));}
  else if(lens.kind==='body'){const r=rowsFor('evolution_corporelle');metrics.push(mtProtocolBodyWeightMetric(r,referenceComparison,'Poids',sourceLabel),mtProtocolNumeric(r,'Tour de taille',v=>mtProtocolNum(v.waist),'cm','range',sourceLabel),mtProtocolNumeric(r,'Confort corporel',v=>mtProtocolNum(v.body_comfort),'/10','average','Ressenti personnel'));}
  else if(lens.kind==='pilates'){const rp=rowsFor('performance_recuperation');metrics.push(mtProtocolNumeric(rp,'Temps pratiqué',v=>mtProtocolNum(v.duration),'min','average','Suivi activité'),mtProtocolCount(rp,'Séances réalisées',v=>mtProtocolNum(v.duration)>0||Boolean(v.session),'Journées avec une séance renseignée.','Suivi activité'),mtProtocolNumeric(rp,'Mobilité / aisance',v=>mtProtocolNum(v.mobility)??mtProtocolNum(v.ease)??mtProtocolNum(v.readiness)??mtProtocolNum(v.availability),'/10','average','Ressenti personnel'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'));}
  else if(lens.kind==='recomposition'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolCount(rp,'Entraînements',v=>mtProtocolNum(v.duration)>0||Boolean(v.session),'Journées avec entraînement renseigné.','Suivi activité'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolBodyWeightMetric(body,referenceComparison,'Poids · plage',sourceLabel),mtProtocolProteinMetric(nutrition,'Protéines documentées'));}
  else if(lens.kind==='mass_gain'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale'),walk=rowsFor('pas_marche');metrics.push(mtProtocolMovementMetric(rp,walk,sourceLabel),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolProteinMetric(nutrition,'Protéines documentées'),mtProtocolBodyWeightMetric(body,referenceComparison,'Poids · plage',sourceLabel));}
  else if(lens.kind==='cramps'){const rp=rowsFor('performance_recuperation'),walk=rowsFor('pas_marche');metrics.push(mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolNumeric(rp,'Inconfort musculaire',v=>mtProtocolNum(v.muscle_discomfort)??mtProtocolNum(v.discomfort)??mtProtocolNum(v.fatigue_after),'/10','average','Ressenti personnel'),mtProtocolNumeric(rp,'Temps de pratique',v=>mtProtocolNum(v.duration),'min','average','Suivi activité'),mtProtocolNumeric(walk,'Pas quotidiens',v=>mtProtocolNum(v.steps),'pas','average',sourceLabel));}
  else if(lens.kind==='silhouette'){const body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale'),rp=rowsFor('performance_recuperation'),walk=rowsFor('pas_marche');metrics.push(mtProtocolBodyWeightMetric(body,referenceComparison,'Poids · plage',sourceLabel),mtProtocolNumeric(body,'Tour de taille · plage',v=>mtProtocolNum(v.waist),'cm','range',sourceLabel),mtProtocolMovementMetric(rp,walk,sourceLabel),mtProtocolProteinMetric(nutrition,'Protéines documentées'));}
  else if(lens.kind==='flat_belly'){const d=rowsFor('digestion'),body=rowsFor('evolution_corporelle');metrics.push(mtProtocolNumeric(d,'Confort digestif',v=>mtProtocolNum(v.comfort),'/10'),mtProtocolNumeric(d,'Ballonnements',v=>mtProtocolNum(v.bloating),'/10'),mtProtocolNumeric(body,'Tour de taille · plage',v=>mtProtocolNum(v.waist),'cm','range',sourceLabel),mtProtocolNumeric(d,'Douleurs / crampes',v=>mtProtocolNum(v.pain),'/10'));}
  else if(lens.kind==='vitality'){const rp=rowsFor('performance_recuperation'),walk=rowsFor('pas_marche'),sleep=rowsFor('sommeil_profond'),nutrition=rowsFor('nutrition_vegetale'),stress=rowsFor('stress_regulation');metrics.push(mtProtocolMovementMetric(rp,walk,sourceLabel),mtProtocolNumeric(sleep,'Sommeil documenté',mtProtocolDuration,'h','average',sourceLabel),mtProtocolNumeric(stress,'Énergie ressentie',v=>mtProtocolNum(v.energy),'/10','average','Ressenti personnel'),mtProtocolProteinMetric(nutrition,'Protéines documentées'));}
  else if(lens.kind==='mobility'){const rp=rowsFor('performance_recuperation'),walk=rowsFor('pas_marche');metrics.push(mtProtocolMovementMetric(rp,walk,sourceLabel),mtProtocolNumeric(rp,'Mobilité / aisance',v=>mtProtocolNum(v.mobility)??mtProtocolNum(v.ease)??mtProtocolNum(v.readiness)??mtProtocolNum(v.availability),'/10','average','Ressenti personnel'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'));}
  else if(lens.kind==='food_balance'){const n=rowsFor('nutrition_vegetale'),eq=rowsFor('equilibre_alimentaire'),d=rowsFor('digestion');metrics.push(mtProtocolProteinMetric(n,'Protéines documentées'),mtProtocolNumeric(n,'Fibres documentées',v=>mtProtocolNum(v.fiber_g),'g','average','Carnet alimentaire'),mtProtocolNumeric(n,'Sucres documentés',v=>mtProtocolNum(v.sugars_g),'g','average','Carnet alimentaire'),mtProtocolNumeric(eq,'Satiété après repas',v=>mtProtocolNum(v.satiety_after),'/10','average','Carnet alimentaire'));if(!metrics[3]?.count)metrics[3]=mtProtocolNumeric(d,'Confort digestif',v=>mtProtocolNum(v.comfort),'/10');}
  else if(lens.kind==='rhythm'){const rp=rowsFor('performance_recuperation'),walk=rowsFor('pas_marche'),sleep=rowsFor('sommeil_profond');metrics.push(mtProtocolMetric('Journées réalisées',`${done.length} / ${total}`,'Les validations du protocole restent conservées.',done.length,'Protocole'),mtProtocolMovementMetric(rp,walk,sourceLabel),mtProtocolNumeric(sleep,'Sommeil documenté',mtProtocolDuration,'h','average',sourceLabel),mtProtocolMetric('Ressentis notés',String(moods),'Entrées du journal d’humeur de ce protocole.',moods,'Journal d’humeur'));}
  else if(lens.kind==='dental'){const bev=rowsFor('boissons'),sleep=rowsFor('sommeil_profond');metrics.push(mtProtocolMetric('Journées réalisées',`${done.length} / ${total}`,'Les validations déjà enregistrées restent prises en compte.',done.length,'Protocole'),mtProtocolNumeric(bev,'Hydratation renseignée',v=>mtProtocolNum(v.hydration_liters),'L','average','Carnet boissons'),mtProtocolMetric('Ressentis notés',String(moods),'Entrées du journal d’humeur de ce protocole.',moods,'Journal d’humeur'),mtProtocolNumeric(sleep,'Sommeil documenté',mtProtocolDuration,'h','average',sourceLabel));}
  else if(lens.kind==='alcohol'){const r=rowsFor('sommeil_profond');metrics.push(mtProtocolMetric('Journées réalisées',`${done.length} / ${total}`,'Les validations du protocole restent conservées depuis sa date de départ.',done.length,'Protocole'),mtProtocolMetric('Ressentis notés',String(moods),'Entrées du journal d’humeur de ce protocole.',moods,'Journal d’humeur'),mtProtocolNumeric(r,'Durée de sommeil',mtProtocolDuration,'h','average',sourceLabel),mtProtocolNumeric(r,'Qualité du sommeil',v=>mtProtocolNum(v.quality),'/10','average','Ressenti personnel'));}
  else if(lens.kind==='gentle'){const r=rowsFor('sommeil_profond');metrics.push(mtProtocolMetric('Journées réalisées',`${done.length} / ${total}`,'Les validations déjà enregistrées restent prises en compte.',done.length,'Protocole'),mtProtocolMetric('Ressentis notés',String(moods),'Entrées du journal d’humeur de ce protocole.',moods,'Journal d’humeur'),mtProtocolNumeric(r,'Sommeil documenté',mtProtocolDuration,'h','average',sourceLabel));}
  else if(lens.kind==='first_steps'){const referenceDays=new Set(compactDays.map(d=>String(d?.date||'')).filter(Boolean));metrics.push(mtProtocolMetric('Journées réalisées',`${done.length} / ${total}`,'Les validations du parcours restent ton repère principal.',done.length,'Parcours'),mtProtocolMetric('Journées avec repères',String(referenceDays.size),'Carnet, boissons, suivis ou autres données réellement renseignées pendant le parcours.',referenceDays.size,'Méthode Tee'),mtProtocolMetric('Ressentis notés',String(moods),'Ressentis enregistrés dans ce parcours.',moods,'Journal d’humeur'));}
  ...
  return {lens,start,end,rows,metrics:metrics.slice(0,4),documentedDates:[...documentedDates],done,elapsed,sources:[...sources]};
}
function mtProtocolEvidence(metric,elapsed){if(metric?.evidence_mode==='profile')return `<div class="protocol-evidence-caption">Repère connecté depuis Mon profil · aucune fausse journée de pesée n’est créée.</div>`;const ratio=Math.max(0,Math.min(1,(Number(metric.count)||0)/Math.max(1,elapsed))),filled=Math.round(ratio*7);return `<div class="protocol-evidence" aria-label="${safe(`${metric.count||0} journée(s) documentée(s) sur ${elapsed}`)}">${Array.from({length:7},(_,i)=>`<i class="${i<filled?'on':''}"></i>`).join('')}</div><div class="protocol-evidence-caption">Présence des données · ${Number(metric.count)||0}/${elapsed} jour${elapsed>1?'s':''} écoulé${elapsed>1?'s':''}</div>`;}
function mtProtocolComparisonMetric(protocol,period){
  const lens=mtProtocolLens(protocol),p=period||{},pick=(key,label,unit='',inverse=false)=>{const value=mtProtocolNum(p[key]);return value===null?null:{label,value:mtProtocolFormat(value,unit,key==='avg_steps'?0:1),inverse};};
  if(['sleep','gentle','alcohol'].includes(lens.kind))return pick('avg_sleep_hours','Sommeil moyen','h');
  if(['digestion','flat_belly','reflux'].includes(lens.kind))return pick('avg_digestion','Confort digestif','/10');
  if(lens.kind==='stress')return pick('avg_stress','Stress moyen','/10',true);
  if(['activity','pilates','cramps'].includes(lens.kind))return pick('avg_steps','Pas moyens','pas')||pick('avg_recovery','Récupération','/10');
  if(['recomposition','mass_gain','silhouette','body'].includes(lens.kind))return pick('avg_weight_kg','Poids moyen','kg')||pick('avg_protein_g','Protéines moy.','g');
  if(['nutrition','sugar','fast'].includes(lens.kind))return pick('avg_fiber_g','Fibres moy.','g')||pick('avg_protein_g','Protéines moy.','g');
  return pick('avg_energy','Énergie moyenne','/10')||pick('avg_sleep_hours','Sommeil moyen','h');
}
function mtRenderProtocolComparison(protocol,comparison){
  if(!comparison?.available)return '';
  const before=comparison.before||{},during=comparison.during||{},after=comparison.after||{},totalDocs=(Number(before.documented_days)||0)+(Number(during.documented_days)||0)+(Number(after.documented_days)||0);if(totalDocs<2)return '';
  const card=(label,p,muted=false)=>{const days=Number(p?.documented_days)||0,metric=mtProtocolComparisonMetric(protocol,p);return `<div class="protocol-period-card ${muted?'is-muted':''}"><small>${safe(label)}</small><b>${days?`${days} jour${days>1?'s':''} documenté${days>1?'s':''}`:'Pas encore de recul'}</b>${metric?`<span>${safe(metric.label)} · ${safe(metric.value)}</span>`:'<span>Données insuffisantes pour comparer</span>'}</div>`;};
  return `<section class="journey-section protocol-periods"><div class="journey-section-kicker">Avant · Pendant · Après</div><div class="journey-section-title">Le protocole dans ton histoire</div><p class="journey-section-sub">Méthode Tee compare des périodes agrégées, sans transformer une variation en réussite ou en échec. La période « Après » apparaît seulement lorsqu’elle existe réellement.</p><div class="protocol-period-grid">${card('Avant',before)}${card('Pendant',during)}${card('Après',after,!Number(after.documented_days))}</div><div class="protocol-source-note"><strong>Lecture longue durée compacte.</strong> Les journées détaillées ne sont pas retéléchargées pour construire cette comparaison.</div></section>`;
}
function mtRenderProtocolMarkers(protocol,progress,total,model){const lens=model?.lens||mtProtocolLens(protocol),metrics=model?.metrics||[],elapsed=model?.elapsed||Math.max(1,Math.min(total,Number(progress.current_day||1)));return `<section class="journey-section"><div class="journey-section-kicker">Balises du protocole</div><div class="journey-section-title">${safe(lens.title)}</div><p class="journey-section-sub">${safe(lens.subtitle)} Les rubans ci-dessous indiquent combien de journées alimentent chaque repère : ils ne notent pas ton corps ni ta santé.</p><div class="protocol-markers">${metrics.map(metric=>`<article class="protocol-marker"><div class="protocol-marker-head"><div><div class="protocol-marker-label">${safe(metric.label)}</div><div class="protocol-marker-value">${safe(metric.value)}</div></div><div class="protocol-marker-source">${safe(mtProtocolPublicSource(metric.source))}</div></div><div class="protocol-marker-detail">${safe(metric.detail)}</div>${mtProtocolEvidence(metric,elapsed)}</article>`).join('')}</div>${model?.sources?.includes('Apple Santé')?'<div class="protocol-source-note"><strong>Apple Santé connecté quand pertinent.</strong> Les mesures automatiques restent séparées de tes ressentis et sont lues en lecture seule.</div>':''}</section>`;}
function mtRenderProtocolTrajectory(model,progress,total){
  const documented=new Set(model?.documentedDates||[]),done=new Set(mtNormalizeCompletedDays(progress.completed_days)),start=model?.start||mtProtocolStartDate(progress),current=Math.max(1,Math.min(total,Number(progress.current_day||1))),streak=Math.max(0,Number(progress.streak||0));
  const weeks=Math.ceil(total/7),rows=[];
  for(let week=0;week<weeks;week++){
    const cells=[];
    for(let col=0;col<7;col++){
      const day=week*7+col+1;if(day>total){cells.push('<span class="protocol-week-empty"></span>');continue;}
      const date=mtAddProtocolDays(start,day-1),isToday=day===current,isDone=done.has(date),isDoc=documented.has(date),future=day>current,missed=day<current&&!isDone&&!isDoc;
      const cls=[isDone?'done':'',isDoc&&!isDone?'documented':'',isToday?'today':'',future?'future':'',missed?'missed':''].filter(Boolean).join(' ');
      cells.push(`<div class="protocol-week-cell ${cls}" title="Jour ${day}"><div class="protocol-week-dot">${isDone?'✓':isToday?day:''}</div></div>`);
    }
    rows.push(`<div class="protocol-week-row"><b>S${week+1}</b><div class="protocol-week-cells">${cells.join('')}</div></div>`);
  }
  const completed=done.size,historyCopy=Number(progress.current_day||1)>1?'<p class="protocol-history-note"><strong>Historique conservé.</strong> Ton protocole continue depuis sa date de départ : les validations déjà enregistrées avant cette mise à jour restent prises en compte. Les nouveaux repères analytiques apparaissent uniquement quand une donnée compatible existe, sans inventer le passé.</p>':'';
  return `<section class="journey-section protocol-progress-card"><div class="journey-section-kicker">Ma progression</div><div class="protocol-progress-head"><div class="journey-section-title">Jour ${current} · ${total}</div>${streak?`<div class="protocol-streak">✦ ${streak} jour${streak>1?'s':''} de suite</div>`:''}</div><p class="journey-section-sub">Une lecture commune à tous les protocoles, séparée des balises analytiques propres à celui-ci.</p><div class="protocol-week-grid">${rows.join('')}</div><div class="protocol-week-legend"><span><i class="done"></i> Jour validé</span><span><i class="today"></i> Aujourd’hui</span><span><i class="missed"></i> Non validé</span><span><i class="future"></i> À venir</span></div><div class="protocol-progress-summary"><strong>${completed}/${total} jours validés</strong><span>Départ · ${safe(start)}</span></div>${historyCopy}</section>`;
}
...
window.renderProtocolJourney=async function(){
    ...
    if(window.MTPhytoSafety){
      ...
      const safetyIds=[...new Set(todayContent.flatMap(c=>Array.isArray(c?.phyto_ingredient_ids)?c.phyto_ingredient_ids:[]).filter(Boolean))];
      const target=document.querySelector(`#journey-day-${currentDay}`)||document.querySelector('.intention-card')||root;
      window.MTPhytoSafety.decorate(target,safetyText,{position:'first',compact:true,ingredientIds:safetyIds});
    }
