...
function mtProtocolLens(protocol){
  ...
  const explicit=[
    ...
    [/stop sucre|sucre/,{kind:'sugar',keys:['reduction_sucre'],title:'Ton rapport au sucre',subtitle:'Envies, alternatives et journées documentées, sans logique de réussite ou d’échec.',health:[]}],
...
window.renderProtocolJourney=async function(){
    ...
    const progress=await getProgress(protocol); const total=durationDays(protocol); const s=score(progress,total); const intention=INTENTIONS[(Number(progress.current_day||1)-1)%INTENTIONS.length]; const [contents,protocolModel,periodComparison]=await Promise.all([getContents(protocol, progress, admin),mtLoadProtocolModel(user,protocol,progress,total),window.MTReference?.protocol?window.MTReference.protocol(protocol.id).catch(()=>null):Promise.resolve(null)]); const done=mtNormalizeCompletedDays(progress.completed_days); const validated=done.includes(todayKey());
