...
async function mtLoadProtocolModel(user,protocol,progress,total){
  ...
  else if(lens.kind==='recomposition'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolCount(rp,'Entraînements',v=>mtProtocolNum(v.duration)>0||Boolean(v.session),'Journées avec entraînement renseigné.','Suivi activité'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolNumeric(body,'Poids · plage',v=>mtProtocolNum(v.weight),'kg','range',sourceLabel),mtProtocolNumeric(nutrition,'Protéines',v=>mtProtocolNum(v.protein_g),'g','average','Carnet · CIQUAL'));}
  else if(lens.kind==='mass_gain'){const rp=rowsFor('performance_recuperation'),body=rowsFor('evolution_corporelle'),nutrition=rowsFor('nutrition_vegetale');metrics.push(mtProtocolNumeric(rp,'Temps d’entraînement',v=>mtProtocolNum(v.duration),'min','average','Suivi activité'),mtProtocolNumeric(rp,'Récupération',v=>mtProtocolNum(v.recovery),'/10','average','Ressenti personnel'),mtProtocolNumeric(nutrition,'Protéines documentées',v=>mtProtocolNum(v.protein_g),'g','average','Carnet · CIQUAL'),mtProtocolNumeric(body,'Poids · plage',v=>mtProtocolNum(v.weight),'kg','range',sourceLabel));}
