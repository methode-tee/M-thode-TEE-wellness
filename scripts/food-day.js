(function(){
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    async function openNutritionDetail(mealId=null){
      const modal=ensureNutritionModal();modal.innerHTML=`<div class="mt-nutrition-bg"></div><section class="mt-nutrition-sheet"><div class="mt-nutrition-grip"></div><button type="button" class="mt-nutrition-close">×</button><div class="mt-nutrition-kicker">Carnet nutritionnel</div><h2>${mealId?'Détail de ce repas':'Détail de ma journée'}</h2><p class="mt-nutrition-lead">Lecture des données nutritionnelles réellement disponibles pour cette sélection. Une valeur absente reste non documentée et n’est jamais remplacée par zéro.</p><div class="mt-nutrition-loading">Recherche des données réellement disponibles…</div></section>`;modal.classList.add('open');modal.querySelector('.mt-nutrition-bg').onclick=closeNutritionDetail;modal.querySelector('.mt-nutrition-close').onclick=closeNutritionDetail;
      try{
