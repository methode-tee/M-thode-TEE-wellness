...
function renderNav() {
  ...
  nav.innerHTML = (window.MT_NAV || []).map(item => {
    ...
    if (current === "protocols.html") {
      if (category) {
        active = item.href.includes(`category=${category}`);
      } else {
        active = item.href.includes("category=pharmacie_vegetale");
      }
    } else {
    ...
    return `<a class="${active ? "active" : ""}" href="${item.href}"><b>${item.emoji}</b><span>${item.label}</span></a>`;
  }).join("");
}
...
async function renderHomeFeed() {
  ...
  el.innerHTML = posts.map(p => `
    <article class="post-card reveal">
      <div class="post-head"><span class="avatar">T</span><div><strong>Méthode<br>Tee</strong><small>${new Date(p.created_at).toLocaleDateString("fr-FR", {day:"2-digit", month:"2-digit", year:"numeric"})}</small></div><span class="tag">${p.type || "Journal"}</span></div>
