...
async function renderHomeFeed() {
  ...
  el.innerHTML = posts.map(p => `
    <article class="post-card reveal">
      <div class="post-head"><span class="avatar">T</span><div><strong>Méthode<br>Tee</strong><small>${new Date(p.created_at).toLocaleDateString("fr-FR", {day:"2-digit", month:"long", year:"numeric"})}</small></div><span class="tag">${p.type || "Journal"}</span></div>
