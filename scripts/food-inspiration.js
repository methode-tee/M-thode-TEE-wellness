(function(){
  ...
  function mtPriceRefLabel(price){
    if(!price||typeof price!=='object')return '';
    const eurkg=Number(price.eur_per_kg),eurl=Number(price.eur_per_l),unit=Number(price.unit_price_eur);
    if(Number.isFinite(eurkg)&&eurkg>0)return `≈ ${eurkg.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})} €/kg`;
    if(Number.isFinite(eurl)&&eurl>0)return `≈ ${eurl.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})} €/L`;
    if(Number.isFinite(unit)&&unit>0)return `≈ ${unit.toLocaleString('fr-FR',{style:'currency',currency:'EUR'})}/unité`;
    return '';
  }
  function mtOpenPlannerWithSnapshot(snapshot){
    const owned=Array.isArray(snapshot?.owned)?snapshot.owned:inputIngredients(snapshot?.ingredients||lastIngredients||'');
    try{sessionStorage.setItem('mtPlannerPantrySeedV1',owned.join(', '));}catch(e){}
    location.href='tee-next.html?tool=planner&from=composer';
  }
  async function mtDecorateInspirationPrices(snapshot){
    const host=document.getElementById('mtInspirePriceContext');if(!host)return;
    const missing=ensureMissingSuggestions(snapshot?.ingredients||lastIngredients,Array.isArray(snapshot?.missing)?snapshot.missing:[]);
    host.innerHTML=`<div class="mt-inspire-price-head"><div><small>Budget & courses</small><b>Utiliser ce que tu as déjà</b></div><button type="button" id="mtInspirePlanWithThis">Planifier avec ces aliments</button></div><p class="mt-inspire-price-note">TEE peut relier ces compléments aux prix de référence quand ils sont disponibles.</p>`;
    document.getElementById('mtInspirePlanWithThis').onclick=()=>mtOpenPlannerWithSnapshot(snapshot);
    if(!sb||!missing.length)return;
    try{
      const {data,error}=await sb.rpc('mt_price_quote_terms_v1',{p_terms:missing,p_country:'FR',p_region:null});
      if(error)throw error;
      const rows=Array.isArray(data)?data:[];
      const usable=rows.filter(x=>mtPriceRefLabel(x?.price));
      if(!usable.length)return;
      const list=document.createElement('div');list.className='mt-inspire-price-list';
      list.innerHTML=usable.map(x=>`<span><b>${esc(x.term||x.food_name||'Complément')}</b><small>${esc(mtPriceRefLabel(x.price))}${x.price?.source_label?` · ${esc(x.price.source_label)}`:''}</small></span>`).join('');
      host.appendChild(list);
      const note=document.createElement('p');note.className='mt-inspire-price-note';note.textContent='Repères de prix uniquement : Composer avec TEE ne connaît pas encore la quantité exacte de chaque complément.';
