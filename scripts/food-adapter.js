  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    function buildRecommendations(raw,goal,knowledge=[],structured=[],answers=[]){
      ...
      if(p.family==='sweet_bowl'){
        ...
        add(`Conserver la structure de ${p.dishName||'ce plat'}`,'La protéine, le féculent et la partie végétale appartiennent déjà à la composition habituelle. Vérifie seulement que ta version les contient avant d’ajouter quoi que ce soit.','Ce plat est normalement composé de plusieurs fonctions complémentaires.');
        signature=`Je garderais ${p.dishName||'ce plat'} comme un ensemble. Je n’ajouterais rien automatiquement sans regarder la recette réellement servie.`;
