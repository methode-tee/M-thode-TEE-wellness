...
(function(){
  ...
  function reevaluationDecision(state,raw,model,trends,storageKey,date){
    ...
    // V4896616 — le retour de difficulté détaillé intervient réellement dans la réévaluation.
    const feedback=h?.adaptive_feedback||model?.context?.adaptive_feedback||{},hardDays=Math.max(0,Number(feedback?.hard_days)||0),feedbackDays=Math.max(0,Number(feedback?.detailed_days)||0);
    if(feedbackDays>=3&&hardDays>=2&&hardDays/Math.max(1,feedbackDays)>=.4){
      return {...raw,key:'reevaluate',flowPhase:'reevaluate',title:'Simplifier plutôt qu’intensifier',summary:`Le repère « ${state.decision.title} » a été appliqué, mais tes retours montrent qu’il a souvent demandé trop d’effort pour devenir une habitude stable.`,action:'Garde le même objectif général, mais choisis une version plus simple du geste pendant quelques jours avant de réévaluer.',reasons:[`${hardDays} retour${hardDays>1?'s':''} « difficile » sur ${feedbackDays} retour${feedbackDays>1?'s':''} détaillé${feedbackDays>1?'s':''}`,...changes].slice(0,3),horizon:'7 jours avec une version plus simple',cycle:{day:7,total:CYCLE_DAYS,startedOn:state.startedOn,reevaluatedOn:state.reevaluatedOn,appliedDays,appliedToday:!!h?.adaptive_cycle_applied_today},cycleComparison:{previous:state.decision.title,next:state.decision.title,same:true,gate:true},previousDecision:state.decision};
    }
