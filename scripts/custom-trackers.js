// MÉTHODE TEE — V415 · SUIVIS PREMIUM · PROFONDEUR + HISTORIQUE + PERSONNALISATION (lazy-load)
(function(){
  'use strict';
  ...
  function addCSS(){
    ...
    style.textContent=`
      ...
      @media(max-width:520px){.mt-follow-config-grid,.mt-follow-history-highlights{grid-template-columns:1fr}.mt-follow-sheet{left:0;right:0;transform:none;width:100%;height:89dvh;max-height:89dvh;padding:22px 20px calc(30px + env(safe-area-inset-bottom,0px))}.mt-follow-row{grid-template-columns:1fr}.mt-follow-row-actions{justify-content:flex-start}.mt-follow-sheet h2{font-size:38px}}
  ...
  function trackerDailySummary(rawKey,values={},settings={},date=TODAY()){
    ...
    if(key==='sommeil_profond'){
      ...
      if(hours!==null)signals.sleep_minutes=Math.round(hours*60);signals.sleep_quality=sleepQuality;signals.energy=num(values.wake_state);signals.sleepiness=num(values.sleepiness);signals.sleep_awakenings=num(values.awakenings);signals.sleep_latency=num(values.latency);
  ...
  function configHTML(key,settings={}){
    ...
    if(key==='evolution_corporelle'){
      ...
      const opts=[['weight','Poids'],['waist','Tour de taille'],['hips','Tour de hanches'],['chest','Tour de poitrine'],['thigh','Tour de cuisse'],['arm','Tour de bras'],['body_fat','Masse grasse %'],['muscle_mass','Masse musculaire']];
  ...
  async function fetchHistory(rawKey,force=false){
    ...
    const localRows=readLocalHistory(key,'1900-01-01'),c=client();let remoteRows=[];
    if(c&&UID){try{
      const query=c.from('user_tracker_entries').select('tracker_key,entry_date,values,note,updated_at').eq('user_id',UID).eq('tracker_key',key).lte('entry_date',TODAY()).order('entry_date',{ascending:false}).limit(1000);
      const result=await Promise.race([query,new Promise(resolve=>setTimeout(()=>resolve({data:null}),2600))]);
  ...
  function historyHighlights(key,rows){
    ...
    if(key==='sommeil_profond'){const durations=values.map(v=>num(v._sleep_hours)??durationBetween(v.bedtime,v.wake_time)).filter(Number.isFinite);push(durations.length?formatDuration(average(durations)):'—','durée moyenne');push(Math.round((average(values.map(v=>num(v.quality)))||0)*10)/10||'—','qualité moyenne /10');push(values.filter(v=>Number(v.awakenings)>0).length,'nuits avec réveils signalés');}
    ...
    else if(key==='performance_recuperation'){push(values.filter(v=>v.session&&!/Repos/i.test(v.session)).length,'séances / pratiques');push(Math.round(values.reduce((a,v)=>a+(num(v.duration)||0),0))+' min','temps de pratique');push(Math.round((average(values.map(v=>num(v.recovery)))||0)*10)/10||'—','récupération /10');}
  ...
  window.mtAdvancedTrackerHistoryPeriod=async function(days){const modal=root('mtAdvancedTrackerEntry','mt-follow-entry'),key=normalizeKey(modal.dataset.key),n=Number(days);renderHistory(modal,key,await fetchHistory(key),[0,7,28,90].includes(n)?n:7);};
