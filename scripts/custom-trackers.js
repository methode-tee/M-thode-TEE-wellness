// MÉTHODE TEE — V435 · BEST-OF SUIVIS PREMIUM + APPLE SANTÉ (egress borné)
(function(){
  'use strict';
  ...
  const CATEGORIES=[
    ...
    ['mental','Mon équilibre mental','Stress, charge mentale et stratégies de régulation, sans diagnostic.'],
  ...
  const TRACKERS={
    ...
    stress_regulation:{category:'mental',title:'Stress & régulation',description:'Observer tension, charge mentale, humeur et ce qui t’aide à revenir à l’équilibre, avec des repères Apple Santé séparés du ressenti.',fields:[
      section('Ressenti du jour','Quelques secondes suffisent : le but est de voir ton évolution, pas de tout analyser.'),
      field('stress','Stress / tension ressentie','range'),field('mental_load','Charge mentale','range'),field('mood','Humeur','range'),field('energy','Énergie','range'),
      field('physical_tension','Tension physique','range'),field('switch_off','Capacité à décrocher','range'),
      field('main_context','Contexte principal','select',['Travail / études','Vie personnelle','Sommeil / fatigue','Santé / inconfort','Charge quotidienne','Plusieurs facteurs','Je ne sais pas','Autre']),
      section('Ce qui a aidé','Ces actions restent des observations personnelles, pas des prescriptions.'),
      field('regulation','Ce que tu as essayé','select',['Rien de particulier','Respiration','Méditation / pleine conscience','Marche / extérieur','Sport / mouvement','Écriture / journal','Temps calme / déconnexion','Échange avec quelqu’un','Plusieurs choses','Autre']),
      field('regulation_help','Est-ce que cela t’a aidé ?','select',['Oui, clairement','Un peu','Pas vraiment','Je ne sais pas','Non concerné']),
      field('sleep_context','Sommeil ressenti','range'),field('notes_context','Événement ou élément utile à retenir','textarea')
    ]},
    ...
    perimenopause:{category:'hormonal',title:'Périménopause & ménopause',description:'Observer les changements physiques et émotionnels présents aujourd’hui, sans supposer de symptôme ni poser de diagnostic.',fields:[
      ...
      field('libido','Libido / désir (facultatif)','select',['Plus bas que d’habitude','Proche de mon habitude','Plus élevé que d’habitude','Variable','Je ne souhaite pas répondre']),
    ...
    fringales_envies:{category:'alimentation',title:'Fringales & envies',description:'Observe quand les envies apparaissent et ce qui les accompagne, sans jugement.',fields:[
      section('Le signal','Une collation adaptée à ta faim et à ton rythme n’est pas un échec. Tu peux aussi renseigner une envie sans avoir mangé ensuite.'),
      field('urge_presence','Présence des fringales / envies dans la journée','range',null,{min:1,max:5,step:1,minLabel:'Aucune',maxLabel:'Très présentes'}),
      field('urge_time','Moment principal de l’envie (facultatif)','time'),
      field('physical_hunger','Faim physique au moment de l’envie','range',null,{min:1,max:5,step:1,minLabel:'Très faible',maxLabel:'Très forte'}),
      field('urge_intensity','Intensité de l’envie principale','range',null,{min:1,max:5,step:1,minLabel:'Légère',maxLabel:'Difficile à ignorer'}),
      field('apaisement_after','Apaisement après avoir mangé (si concernée)','range',null,{min:1,max:5,step:1,minLabel:'Pas apaisée',maxLabel:'Complètement apaisée'}),
      section('Le contexte','Renseigne seulement ce que Méthode Tee ne peut pas connaître seule. Les repas, le sommeil, le stress, l’activité, le cycle et les autres repères déjà disponibles seront reliés en arrière-plan.'),
      field('main_context','Contexte principal','select',['Faim','Fatigue','Stress','Ennui','Émotion','Habitude','Envie de goût','Social','Je ne sais pas']),
      field('urge_type','Type d’envie (facultatif)','select',['Envie sucrée','Envie salée','Envie d’un aliment précis','Envie de manger sans aliment précis','Variable','Je ne sais pas']),
      field('eating_response','Ce qui s’est passé ensuite (facultatif)','select',['Envie seulement, je n’ai pas mangé','Collation prévue / structurée','Prise alimentaire spontanée','Repas','Je préfère ne pas préciser'])
    ]},
  ...
  function trackerSummary(key,values={},settings={},date=TODAY()){
    ...
    if(key==='fringales_envies')return [present(values.urge_presence)?`envies ${values.urge_presence}/5`:'',present(values.urge_intensity)?`intensité ${values.urge_intensity}/5`:'',present(values.physical_hunger)?`faim ${values.physical_hunger}/5`:'',values.main_context||'',/Collation prévue/.test(String(values.eating_response||''))?'collation structurée':''].filter(Boolean).join(' · ')||'Envies renseignées';
  ...
  function trackerDailySummary(rawKey,values={},settings={},date=TODAY()){
    ...
    if(key==='stress_regulation'){
      const stress=num(values.stress),load=num(values.mental_load),hrv=num(values._healthkit_hrv_ms),rhr=num(values._healthkit_resting_hr_bpm);
      pill('Stress',stress===null?'renseigné':`${stress}/10`);if(present(values.regulation))pill('Régulation',values.regulation);
      metric('Stress',stress===null?'':`${stress}/10`);metric('Charge mentale',load===null?'':`${load}/10`);metric('Humeur',present(values.mood)?`${values.mood}/10`:'');metric('Ce qui a aidé',values.regulation);metric('HRV Apple Santé',hrv===null?'':`${hrv} ms`);metric('FC repos Apple Santé',rhr===null?'':`${rhr} bpm`);
      signals.stress=stress;signals.mental_load=load;signals.mood=num(values.mood);signals.energy=num(values.energy);signals.sleep_quality=num(values.sleep_context);signals.regulation=values.regulation||null;signals.hrv_ms=hrv;signals.resting_hr_bpm=rhr;signals.daylight_minutes=num(values._healthkit_daylight_minutes);
      ...
      metric('Activité',discipline);metric('Séance',values.session);metric('Durée',values.duration?`${values.duration} min`:'');metric('Pas',present(values._healthkit_steps)?new Intl.NumberFormat('fr-FR').format(Number(values._healthkit_steps)):'');metric('Distance',present(values._healthkit_distance_km)?`${String(values._healthkit_distance_km).replace('.',',')} km`:'');metric('Énergie active',present(values._healthkit_active_energy_kcal)?`${String(values._healthkit_active_energy_kcal).replace('.',',')} kcal`:'');metric('Intensité',present(values.intensity)?`${values.intensity}/10`:'');metric('Énergie',present(values.energy_before)?`${values.energy_before}/10`:'');metric('Récupération',present(values.recovery)?`${values.recovery}/10`:'');metric('HRV Apple Santé',present(values._healthkit_hrv_ms)?`${values._healthkit_hrv_ms} ms`:'');metric('FC repos Apple Santé',present(values._healthkit_resting_hr_bpm)?`${values._healthkit_resting_hr_bpm} bpm`:'');
      signals.discipline=discipline;signals.sport_duration=num(values.duration);signals.sport_intensity=num(values.intensity);signals.energy=num(values.energy_before);signals.recovery=num(values.recovery);signals.readiness=num(values.readiness)??num(values.availability);signals.sleep_quality=num(values.sleep_quality)??num(values.sleep);signals.fatigue=num(values.fatigue_after)??num(values.muscle_fatigue);signals.steps=num(values._healthkit_steps);signals.distance_km=num(values._healthkit_distance_km);signals.active_energy_kcal=num(values._healthkit_active_energy_kcal);signals.workout_minutes=num(values._healthkit_workout_minutes);signals.hrv_ms=num(values._healthkit_hrv_ms);signals.resting_hr_bpm=num(values._healthkit_resting_hr_bpm);signals.hr_recovery_bpm=num(values._healthkit_hr_recovery_bpm);signals.vo2max=num(values._healthkit_vo2max);signals.respiratory_rate=num(values._healthkit_respiratory_rate);signals.daylight_minutes=num(values._healthkit_daylight_minutes);
      ...
      metric('Aujourd’hui',values.day_state);metric('Phase',phase);metric('Énergie',present(values.energy)?`${values.energy}/10`:'');metric('Douleurs',present(values.pain)?`${values.pain}/10`:'');metric('Appétit',present(values.appetite)?`${values.appetite}/10`:'');metric('Flux',values.flow);metric('Température basale Apple Santé',present(values._healthkit_basal_temp_c)?`${values._healthkit_basal_temp_c} °C`:'');
      signals.cycle_day=cycleDay;signals.cycle_phase=phase;signals.cycle_event=cycleEvent;signals.cycle_state=values.day_state||null;signals.energy=num(values.energy);signals.pain=num(values.pain);signals.appetite=num(values.appetite);signals.sleep_quality=num(values.sleep);signals.mood=num(values.mood);signals.basal_temperature_c=num(values._healthkit_basal_temp_c);signals.healthkit_menstrual_flow=values._healthkit_menstrual_flow||null;signals.healthkit_spotting=values._healthkit_spotting||null;
    ...
    }else if(key==='fringales_envies'){
      const presence=num(values.urge_presence),urge=num(values.urge_intensity),hunger=num(values.physical_hunger),planned=/Collation prévue \/ structurée/.test(String(values.eating_response||''));
      pill('Envies',presence===null?'renseignées':`${presence}/5`);if(planned)pill('Réponse','collation structurée');
      metric('Présence',presence===null?'':`${presence}/5`);metric('Intensité',urge===null?'':`${urge}/5`);metric('Faim physique',hunger===null?'':`${hunger}/5`);metric('Contexte',values.main_context);metric('Réponse',values.eating_response);metric('Apaisement',present(values.apaisement_after)?`${values.apaisement_after}/5`:'');
      signals.craving_presence=presence;signals.craving_intensity=urge;signals.physical_hunger=hunger;signals.craving_context=values.main_context||null;signals.craving_type=values.urge_type||null;signals.craving_response=values.eating_response||null;signals.structured_snack=planned;signals.apaisement_after=num(values.apaisement_after);signals.previous_sleep_hours=num(values._previous_sleep_hours);signals.protein_g=num(values._protein_g);signals.fiber_g=num(values._fiber_g);signals.cycle_phase=values._cycle_phase||null;
  ...
  function fieldHTML(def,values={}){
    ...
    const rangeMin=Number.isFinite(Number(def.attrs?.min))?Number(def.attrs.min):0,rangeMax=Number.isFinite(Number(def.attrs?.max))?Number(def.attrs.max):10,rangeStep=Number.isFinite(Number(def.attrs?.step))?Number(def.attrs.step):1,rangeDenom=rangeMax,rangeDefault=Math.round((rangeMin+rangeMax)/2),current=val(values,def.key,def.type==='range'?rangeDefault:'');
    const rangeEnds=(def.attrs?.minLabel||def.attrs?.maxLabel)?`<div class="mt-follow-range-ends"><small>${esc(def.attrs?.minLabel||rangeMin)}</small><small>${esc(def.attrs?.maxLabel||rangeMax)}</small></div>`:'';
    // Un curseur masqué ne doit jamais devenir implicitement une valeur médiane. Tous
    // les ressentis sont volontaires, sauf si un futur champ déclare required.
    if(def.type==='range'&&def.attrs?.required!==true){const active=present(values[def.key]);return `<div class="mt-follow-field mt-follow-optional-range" data-optional-range><div class="mt-follow-optional-head"><label>${esc(def.label)} <small>(facultatif)</small></label><button class="mt-follow-optional-toggle" type="button" onclick="mtAdvancedTrackerOptionalRangeToggle(this)">${active?'Ne pas renseigner':'Renseigner'}</button></div><div class="mt-follow-range" ${active?'':'hidden'}><input name="${esc(def.key)}" type="range" min="${esc(rangeMin)}" max="${esc(rangeMax)}" step="${esc(rangeStep)}" value="${esc(current)}" ${active?'':'disabled'} data-range-denom="${esc(rangeDenom)}" oninput="this.nextElementSibling.value=this.value+'/${rangeDenom}'"><output>${esc(current)}/${esc(rangeDenom)}</output>${rangeEnds}</div></div>`;}
    if(def.type==='range')return `<div class="mt-follow-field"><label>${esc(def.label)}</label><div class="mt-follow-range"><input name="${esc(def.key)}" type="range" min="${esc(rangeMin)}" max="${esc(rangeMax)}" step="${esc(rangeStep)}" value="${esc(current)}" data-range-denom="${esc(rangeDenom)}" oninput="this.nextElementSibling.value=this.value+'/${rangeDenom}'"><output>${esc(current)}/${esc(rangeDenom)}</output>${rangeEnds}</div></div>`;
  ...
  function plannedFastHours(value){const m=String(value||'').match(/(12|13|14|16|18)/);return m?Number(m[1]):null;}
  function currentFastHours(lastMeal,date=TODAY()){if(date!==TODAY()||!/^[0-2]\d:[0-5]\d$/.test(String(lastMeal||'')))return null;const [h,m]=lastMeal.split(':').map(Number),start=new Date();start.setHours(h,m,0,0);let diff=(Date.now()-start.getTime())/3600000;if(diff<0)diff+=24;return Math.max(0,Math.min(24,diff));}
  function fastingLiveHTML(key,values={}){if(key!=='jeune_intermit')return '';const observed=durationBetween(values.last_meal,values.first_meal),live=!values.first_meal?currentFastHours(values.last_meal):null,target=plannedFastHours(values.planned_window),hours=observed??live;if(hours===null)return `<div class="mt-fast-live" data-fast-live><small>Jeûne · repère simple</small><b>À ton rythme</b><p>Renseigne ton dernier repas pour voir la durée évoluer. Rien n’est enregistré automatiquement.</p></div>`;const pct=target?Math.max(0,Math.min(100,hours/target*100)):Math.min(100,hours/18*100),label=observed!==null?'FENÊTRE OBSERVÉE':'EN COURS',finish=target&&live!==null?(()=>{const d=new Date(Date.now()+(target-live)*3600000);return d.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});})():'';return `<div class="mt-fast-live" data-fast-live><small>${label}</small><b>${String(Math.floor(hours)).padStart(2,'0')} h ${String(Math.round((hours%1)*60)).padStart(2,'0')}</b><div class="mt-fast-track"><i style="width:${pct.toFixed(0)}%"></i></div><p>${target?`${fmtMetric(hours,'h',1)} sur ${target} h${finish?` · fin estimée ${finish}`:''}`:'Durée observée · sans cible imposée'}</p></div>`;}
  function refreshFastingLive(){const host=document.querySelector('[data-fast-live]'),form=document.getElementById('mtAdvancedTrackerForm');if(!host||!form)return;const values={last_meal:form.elements.last_meal?.value||'',first_meal:form.elements.first_meal?.value||'',planned_window:form.elements.planned_window?.value||''};host.outerHTML=fastingLiveHTML('jeune_intermit',values);}
  const SKIN_DB='mt_private_skin_v1';
  function skinPhotoHTML(key,date){if(key!=='peau')return '';return `<div class="mt-skin-photo" data-skin-photo><b>Photo repère privée</b><p class="mt-follow-intro" style="margin:4px 0 8px">Facultative · conservée uniquement sur cet appareil pour comparer ton évolution.</p><div data-skin-photo-preview></div><input type="file" accept="image/*" capture="user" data-skin-photo-input hidden><button type="button" onclick="mtTrackerChooseSkinPhoto()">Ajouter / remplacer la photo</button></div>`;}
  async function skinDB(){return new Promise((resolve,reject)=>{const req=indexedDB.open(SKIN_DB,1);req.onupgradeneeded=()=>{if(!req.result.objectStoreNames.contains('photos'))req.result.createObjectStore('photos')};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
  async function skinPut(date,file){const db=await skinDB();return new Promise((resolve,reject)=>{const tx=db.transaction('photos','readwrite');tx.objectStore('photos').put(file,`${UID||'guest'}:${date}`);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});}
  async function skinGet(date){try{const db=await skinDB();return await new Promise((resolve,reject)=>{const tx=db.transaction('photos','readonly'),r=tx.objectStore('photos').get(`${UID||'guest'}:${date}`);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>reject(r.error);});}catch(_){return null;}}
  window.mtTrackerChooseSkinPhoto=function(){document.querySelector('[data-skin-photo-input]')?.click();};
  async function hydrateSkinPhoto(date){const box=document.querySelector('[data-skin-photo]'),input=box?.querySelector('[data-skin-photo-input]'),preview=box?.querySelector('[data-skin-photo-preview]');if(!box||!input||!preview)return;const render=file=>{if(!file){preview.innerHTML='';return;}const u=URL.createObjectURL(file);preview.innerHTML=`<img src="${u}" alt="Photo repère privée de peau">`;};render(await skinGet(date));input.onchange=async()=>{const f=input.files?.[0];if(!f)return;if(f.size>7*1024*1024){toast('Choisis une photo de moins de 7 Mo.');return;}await skinPut(date,f);render(f);toast('Photo conservée uniquement sur cet appareil.');};}
  const MEAL_CONTEXT_CACHE=new Map();
  async function recentMealContext(date){const key=`${UID||'guest'}:${date}`,cached=MEAL_CONTEXT_CACHE.get(key);if(cached&&Date.now()-cached.at<5*60*1000)return cached.rows;const c=client();if(!c||!UID)return[];try{const from=addDays(date,-1),q=c.from('food_meals').select('meal_date,meal_time,meal_type,description,source_recipe_title').eq('user_id',UID).gte('meal_date',from).lte('meal_date',date).order('meal_date',{ascending:false}).order('meal_time',{ascending:false}).limit(6),r=await Promise.race([q,new Promise(resolve=>setTimeout(()=>resolve({data:[]}),2200))]),rows=Array.isArray(r?.data)?r.data:[];MEAL_CONTEXT_CACHE.set(key,{at:Date.now(),rows});return rows;}catch(_){return[];}}
  function contextAssistHTML(key,date){if(!['digestion','reflux','jeune_intermit'].includes(key))return '';return `<div class="mt-context-assist" data-meal-context><b>Relier à mon Carnet</b><p>Facultatif. Aucune lecture alimentaire supplémentaire ne part tant que tu n’appuies pas ici.</p><button type="button" onclick="mtTrackerUseRecentMeal('${esc(key)}','${esc(date)}')">Reprendre un repas récent</button><div data-meal-context-result></div></div>`;}
  window.mtTrackerUseRecentMeal=async function(key,date){const box=document.querySelector('[data-meal-context]'),out=box?.querySelector('[data-meal-context-result]');if(!box||!out)return;out.innerHTML='<p>Lecture du dernier repas…</p>';const rows=await recentMealContext(date);if(!rows.length){out.innerHTML='<p>Aucun repas récent trouvé dans le Carnet.</p>';return;}const row=rows[0],label=[row.source_recipe_title||row.description||'Repas',row.meal_time?String(row.meal_time).slice(0,5):''].filter(Boolean).join(' · '),form=document.getElementById('mtAdvancedTrackerForm');if(key==='digestion'&&form?.elements.foods)form.elements.foods.value=label;if(key==='reflux'&&form?.elements.previous_meal)form.elements.previous_meal.value=label;if(key==='jeune_intermit'&&form?.elements.last_meal&&row.meal_time)form.elements.last_meal.value=String(row.meal_time).slice(0,5);out.innerHTML=`<p><b>${esc(label)}</b><br>Repère repris et modifiable avant enregistrement.</p>`;if(key==='jeune_intermit')refreshFastingLive();};
  function cyclePhaseForRow(row,settings){return row?.values?.cycle_phase_estimate||cycleEstimate(settings,row?.entry_date)?.phase||'';}
  function phasePatternHTML(rows,settings){if(rows.length<6)return '';const metrics=[['appetite','Appétit'],['energy','Énergie'],['bloating','Ballonnements'],['sleep','Sommeil'],['mood','Humeur']],phases=new Map();rows.forEach(row=>{const phase=cyclePhaseForRow(row,settings);if(!phase)return;const bucket=phases.get(phase)||{};metrics.forEach(([k])=>{const n=num(row.values?.[k]);if(n!==null)(bucket[k]||(bucket[k]=[])).push(n)});phases.set(phase,bucket);});const current=cycleEstimate(settings,TODAY())?.phase;if(!current||!phases.has(current))return '';let best=null;metrics.forEach(([k,label])=>{const own=phases.get(current)?.[k]||[],other=[];phases.forEach((b,p)=>{if(p!==current)other.push(...(b[k]||[]))});if(own.length>=2&&other.length>=3){const a=average(own),b=average(other),d=a-b;if(Math.abs(d)>=.7&&(!best||Math.abs(d)>Math.abs(best.d)))best={label,d,count:own.length};}});if(!best)return `<div class="mt-premium-read"><small>CE QUI ÉVOLUE CHEZ TOI</small><h3>${esc(current)}</h3><p>Continue à renseigner quelques cycles : Méthode Tee comparera tes propres repères par phase, sans transformer une estimation en règle.</p></div>`;const direction=best.d>0?'plus élevé':'plus bas';return `<div class="mt-premium-read"><small>CE QUI ÉVOLUE CHEZ TOI</small><h3>${esc(best.label)} généralement ${direction}</h3><p>Pendant ${esc(current.toLowerCase())}, ton ${esc(best.label.toLowerCase())} renseigné est en moyenne ${direction} que sur tes autres phases disponibles (${best.count} journées comparables). C’est une tendance personnelle, pas une règle biologique ni un outil contraceptif.</p></div>`;}
  function bodySmoothedHTML(rows,settings){const series=metricSeries(rows,v=>num(v.weight));if(series.length<4)return '';const smooth=series.map((p,i)=>({date:p.date,value:average(series.slice(Math.max(0,i-2),i+1).map(x=>x.value))})),first=smooth[0],last=smooth.at(-1),delta=last.value-first.value;if(Math.abs(delta)<.05)return `<div class="mt-premium-read"><small>TENDANCE LISSÉE</small><h3>Plutôt stable</h3><p>La lecture lissée atténue les variations d’une pesée isolée. Ton intention reste : ${esc(settings.body_intention||'observer ton évolution')}.</p></div>`;const sign=delta>0?'+':'',word=delta>0?'hausse progressive':'baisse progressive';return `<div class="mt-premium-read"><small>TENDANCE LISSÉE · ${series.length} mesures</small><h3>${esc(word)}</h3><p>${fmtMetric(first.value,'kg',1)} → ${fmtMetric(last.value,'kg',1)} · ${sign}${fmtMetric(delta,'kg',1)}. Une baisse n’est jamais automatiquement un progrès et une hausse jamais un échec : la lecture dépend de ton intention.</p></div>`;}
  function habitConsistencyHTML(rows){if(!rows.length)return '';const dateSet=new Set(rows.filter(r=>/Petit pas réalisé/i.test(String(r.values?.day_state||''))||String(r.values?.victory||'').trim()).map(r=>r.entry_date)),last7=Array.from({length:7},(_,i)=>addDays(TODAY(),i-6)),on7=last7.filter(d=>dateSet.has(d)).length,last30=Array.from({length:30},(_,i)=>addDays(TODAY(),i-29)),on30=last30.filter(d=>dateSet.has(d)).length;return `<div class="mt-premium-read"><small>RÉGULARITÉ · SANS STREAK PUNITIF</small><h3>${on7} jours sur 7</h3><div class="mt-premium-dots">${last7.map(d=>`<i class="mt-premium-dot ${dateSet.has(d)?'is-on':''}" title="${esc(d)}"></i>`).join('')}</div><p>${on30}/30 derniers jours avec un petit pas renseigné. Une journée vide ne remet pas les précédentes à zéro.</p></div>`;}
  function micronutritionReportHTML(rows){if(!rows.length)return '';const days=rows.length,keys=['iron_mg','magnesium_mg','vitamin_b12_ug','vitamin_b9_ug','vitamin_d_ug','zinc_mg'],items=[];keys.forEach(k=>{const meta=MICRO_META[k];if(!meta)return;const vals=rows.map(r=>microNumber(r.values||{},k)).filter(Number.isFinite);if(vals.length)items.push([meta[0],vals.length,average(vals),meta[1]]);});const fiber=rows.map(r=>num(r.values?.fiber_g)).filter(Number.isFinite);if(fiber.length)items.push(['Fibres',fiber.length,average(fiber),'g']);if(!items.length)return '';return `<div class="mt-premium-read"><small>RAPPORT MICRONUTRITIONNEL</small><h3>${days} journées disponibles</h3><p>Couverture documentaire : une journée sans valeur disponible reste « non documentée », jamais zéro.</p><div class="mt-premium-micro">${items.slice(0,7).map(([label,count,av,unit])=>`<div class="mt-premium-micro-row"><span>${esc(label)}</span><b>${count}/${days} j · moy. ${esc(fmtMetric(av,unit,1))}</b></div>`).join('')}</div></div>`;}
  function recoveryBaselineHTML(rows){const specs=[['_healthkit_hrv_ms','HRV','ms'],['_healthkit_resting_hr_bpm','FC au repos','bpm'],['_healthkit_hr_recovery_bpm','Récupération cardiaque','bpm'],['_healthkit_vo2max','Cardio fitness','ml/kg/min']],points=[];specs.forEach(([k,label,unit])=>{const series=metricSeries(rows,v=>num(v[k]));if(series.length<4)return;const latest=series.at(-1).value,base=median(series.slice(0,-1).map(x=>x.value));if(base===null)return;const pct=(latest-base)/Math.max(Math.abs(base),1)*100;if(Math.abs(pct)>=5)points.push({label,unit,latest,base,pct});});if(!points.length)return '';const p=points.sort((a,b)=>Math.abs(b.pct)-Math.abs(a.pct))[0],different=Math.abs(p.pct)>=10?'diffère nettement':'diffère légèrement';return `<div class="mt-premium-read"><small>PAR RAPPORT À TON PROPRE RYTHME</small><h3>${esc(p.label)} : ${different}</h3><p>Aujourd’hui ${esc(fmtMetric(p.latest,p.unit,1))} · repère personnel récent ${esc(fmtMetric(p.base,p.unit,1))}. Méthode Tee n’utilise pas cette valeur seule pour décider si ta récupération est bonne ou mauvaise : elle reste un contexte à croiser avec sommeil, ressenti et activité.</p></div>`;}
  function perimenopauseMapHTML(rows){if(rows.length<4)return '';const specs=[['hot_flashes','Bouffées de chaleur',v=>!/Aucune/i.test(String(v||''))&&!/^Je ne/.test(String(v||''))],['night_sweats','Sueurs nocturnes',v=>!/Aucune/i.test(String(v||''))],['vaginal_dryness','Confort intime',v=>!/^Non$|Je ne/i.test(String(v||''))],['bleeding_change','Changements de saignement',v=>String(v||'')==='Oui'],['palpitations','Palpitations ressenties',v=>/^Oui/.test(String(v||''))]],hits=[];specs.forEach(([k,label,test])=>{const n=rows.filter(r=>test(r.values?.[k])).length;if(n)hits.push([label,n]);});if(!hits.length)return '';hits.sort((a,b)=>b[1]-a[1]);return `<div class="mt-premium-read"><small>CARTE DE MES CHANGEMENTS</small><h3>${esc(hits[0][0])} revient le plus souvent</h3><div class="mt-premium-points">${hits.slice(0,4).map(([l,n])=>`<div class="mt-premium-point"><b>${n}/${rows.length}</b><small>${esc(l)}</small></div>`).join('')}</div><p style="margin-top:10px">Lecture descriptive uniquement : la fréquence d’un symptôme ne constitue pas un diagnostic.</p></div>`;}
  function premiumInsightHTML(key,rows,settings,days){if(key==='cycle')return phasePatternHTML(rows,settings);if(key==='evolution_corporelle')return bodySmoothedHTML(rows,settings);if(key==='changer_habitude')return habitConsistencyHTML(rows);if(key==='nutrition_vegetale')return micronutritionReportHTML(rows);if(key==='performance_recuperation')return recoveryBaselineHTML(rows);if(key==='perimenopause')return perimenopauseMapHTML(rows);return '';}
  const GLOBAL_TRENDS_CACHE=new Map();
  function rowsFromReferenceOverview(data){
    const rows=[];(data?.days||[]).forEach(day=>{
      ...
      if(present(core.digestion))ensure('digestion',{comfort:core.digestion});
  ...
  async function globalTrackerRows(days=28){
    const n=Math.min(90,Math.max(7,Number(days)||28)),key=`${UID||'guest'}:${n}`,cached=GLOBAL_TRENDS_CACHE.get(key);if(cached&&Date.now()-cached.at<5*60*1000)return cached.rows;
    // V441 : les tendances transversales ne téléchargent plus l'historique brut des suivis.
    // Si l'agrégateur compact n'est pas encore installé, on préfère ne rien afficher plutôt
    // que déclencher un fallback coûteux en cached egress.
    if(n<=28&&window.MTReference?.overview){try{const compact=await window.MTReference.overview('28d');const rows=rowsFromReferenceOverview(compact);GLOBAL_TRENDS_CACHE.set(key,{at:Date.now(),rows});return rows;}catch(e){console.warn('[Mes tendances] agrégats compacts indisponibles',e);}}
    GLOBAL_TRENDS_CACHE.set(key,{at:Date.now(),rows:[]});return[];
  }
  function pairByDate(rows,xKey,xGet,yKeys,yGet){const by=new Map();rows.forEach(r=>{const a=by.get(r.entry_date)||[];a.push(r);by.set(r.entry_date,a)});const pairs=[];by.forEach(day=>{const xr=day.find(r=>r.tracker_key===xKey),ys=day.filter(r=>yKeys.includes(r.tracker_key));const x=xr?xGet(xr.values||{}):null,y=ys.map(r=>yGet(r.values||{})).filter(Number.isFinite);if(Number.isFinite(x)&&y.length)pairs.push([x,average(y)]);});return pairs;}
  function relationTrend(title,copy,pairs){const r=correlation(pairs);if(r===null||Math.abs(r)<.3)return null;return {title,copy:`${copy} sur ${pairs.length} journées comparables. Les tendances montrent des associations dans tes données, pas des causes certaines.`};}
  function temporalRefluxTrend(trackerRows=[]){const reflux=trackerRows.filter(r=>r.tracker_key==='reflux'&&!/^Non/i.test(String(r.values?.episode||'')));if(reflux.length<3)return null;const documented=reflux.filter(r=>Number.isFinite(Number(r.values?._after_meal_3h)));if(documented.length<3)return null;const matched=documented.filter(r=>Number(r.values?._after_meal_3h)===1).length;if(matched<3)return null;return {title:'Repas → reflux : ordre temporel observé',copy:`${matched} épisode${matched>1?'s':''} de reflux sur ${documented.length} renseigné${documented.length>1?'s':''} sont survenus dans les 3 h suivant un repas enregistré. Cela ne prouve pas qu’un aliment précis en est la cause.`};}
  function trendPeriodButtons(days){return `<div class="mt-follow-history-head"><button class="mt-follow-period ${days===28?'is-on':''}" type="button" onclick="mtAdvancedTrackerGlobalTrends(28)">28 jours</button><button class="mt-follow-period ${days===90?'is-on':''}" type="button" onclick="mtAdvancedTrackerGlobalTrends(90)">3 mois</button></div>`;}
  function monthlyValue(month,key){const x=month?.metrics?.[key];return x&&Number.isFinite(Number(x.avg))?Number(x.avg):null;}
  function longTermCards(data){
    const months=(data?.months||[]).filter(m=>Number(m.days_documented)>0).sort((a,b)=>String(a.month).localeCompare(String(b.month))),cards=[];if(months.length<2)return cards;const first=months[0],last=months.at(-1);
    const add=(cat,title,key,unit='',digits=1,neutral=false)=>{const a=monthlyValue(first,key),b=monthlyValue(last,key);if(a===null||b===null)return false;const delta=b-a,stable=Math.abs(delta)<(key==='steps'?350:key==='weight_kg'?0.5:0.35);const direction=stable?'plutôt stable':neutral?'a évolué':delta>0?'est plus haut':'est plus bas';cards.push([cat,{title,copy:`Moyenne mensuelle : ${fmtMetric(a,unit,digits)} → ${fmtMetric(b,unit,digits)}. Le repère ${direction} sur les mois documentés ; aucune causalité n’est déduite.`}]);return true;};
    const firstAvailable=(cat,specs)=>{for(const spec of specs)if(add(cat,...spec))return;};
    firstAvailable('Récupération',[[ 'Sommeil · tendance 3 mois','sleep_hours','h',1,false],[ 'Récupération · tendance 3 mois','recovery','/10',1,true]]);
    firstAvailable('Mental',[[ 'Stress · tendance 3 mois','stress','/10',1,true],[ 'Humeur · tendance 3 mois','mood','/10',1,true]]);
  ...
  function holisticTrendHTML(model){
    const h=model?.context?.holistic;if(!h)return '';const blocks=[];
    const c=h.concordance||{},cr=h.cravings||{},assoc=h.associations||{};
    if(c.state==='mixed')blocks.push(`<div class="mt-global-trend"><small>CONTEXTE RÉCENT</small><b>Des repères évoluent différemment</b><p>Certains signaux sont plus fragiles que d’habitude tandis que d’autres restent favorables. Méthode Tee continue d’observer avant de choisir une priorité.</p></div>`);
    else if(Number(c.recovery_load_count)>=2)blocks.push(`<div class="mt-global-trend"><small>CONTEXTE RÉCENT</small><b>Plusieurs repères vont dans le même sens</b><p>Ton sommeil, ton stress, ton énergie ou ta récupération se sont déplacés ensemble par rapport à ton rythme habituel. Méthode Tee protège d’abord ce terrain avant de proposer un autre ajustement.</p></div>`);
    const high=Number(cr.high_days)||0;if(high>=3){
      const candidates=[['après des nuits plus courtes',Number(cr.high_after_shorter_sleep)||0],['les jours de stress plus élevé',Number(cr.high_with_high_stress)||0],['les jours où les protéines documentées sont plus basses',Number(cr.high_with_lower_protein)||0],['les jours où les fibres documentées sont plus basses',Number(cr.high_with_lower_fiber)||0]].sort((a,b)=>b[1]-a[1]);
      const best=candidates[0];if(best?.[1]>=Math.max(2,Math.ceil(high*.6)))blocks.push(`<div class="mt-global-trend"><small>FRINGALES & ENVIES</small><b>Un contexte commence à se répéter</b><p>Sur tes journées comparables avec envie marquée, ce signal apparaît souvent ${esc(best[0])}. On continue à l’observer : une association répétée ne prouve pas une cause.</p></div>`);
      else if(Number(cr.meal_gap_days)>=3&&Number(cr.avg_minutes_since_previous_meal)>=240)blocks.push(`<div class="mt-global-trend"><small>FRINGALES & ENVIES</small><b>L’espacement des repas mérite d’être observé</b><p>Plusieurs envies marquées ont été renseignées après un intervalle d’au moins quatre heures depuis le repas précédent. Cela peut correspondre à une vraie faim : on ne cherche pas à supprimer automatiquement ce moment.</p></div>`);
    }
    const labels={sleep_previous_to_energy:'Sommeil précédent ↔ énergie du lendemain',sleep_previous_to_cravings:'Sommeil précédent ↔ fringales du lendemain',stress_to_digestion:'Stress ↔ digestion',activity_to_recovery:'Activité ↔ récupération',activity_to_next_recovery:'Activité ↔ récupération du lendemain',activity_to_next_sleep:'Activité ↔ sommeil suivant',protein_to_satiety:'Protéines ↔ satiété',fiber_to_digestion:'Fibres ↔ digestion',fast_duration_to_hunger:'Durée du jeûne ↔ faim',fast_duration_to_energy:'Durée du jeûne ↔ énergie',fast_duration_to_digestion:'Durée du jeûne ↔ digestion',cycle_appetite_to_cravings:'Appétit du cycle ↔ fringales & envies',perimenopause_hot_flashes_to_cravings:'Bouffées de chaleur ↔ fringales & envies',perimenopause_night_sweats_to_cravings:'Sueurs nocturnes ↔ fringales & envies'};
    if(blocks.length<2){for(const [k,label] of Object.entries(labels)){const a=assoc[k]||{},pairs=Number(a.pairs)||0,r=Number(a.r);if(pairs>=7&&Number.isFinite(r)&&Math.abs(r)>=.45){blocks.push(`<div class="mt-global-trend"><small>LIEN À OBSERVER</small><b>${esc(label)}</b><p>Ces deux repères évoluent régulièrement ensemble dans tes journées comparables. Méthode Tee le garde comme piste personnelle, jamais comme preuve d’une cause.</p></div>`);break;}}}
    return blocks.slice(0,2).join('');
  }

  window.mtAdvancedTrackerGlobalTrends=async function(days=28){
    days=Number(days)===90?90:28;addCSS();if(!UID)UID=(await getUser())?.id||window.__MT_LIBRARY_USER_ID__||null;const modal=root('mtAdvancedTrackerEntry','mt-follow-entry');modal.classList.add('open');modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">Méthode Tee · transversal</div><h2><span class="mt-simple-star-inline" aria-hidden="true">✷</span>Mes tendances</h2>${trendPeriodButtons(days)}<div class="mt-follow-loading"><b>Lecture de mes repères</b><p>${days===90?'Lecture des agrégats mensuels uniquement.':'Lecture fine de 28 jours, sans historique illimité.'}</p><span></span></div></section>`;
    let cards=[],model=null,adaptiveRows=[];
    if(days===90&&window.MTReference?.overview){const [longData,ctx]=await Promise.all([window.MTReference.overview('3m'),window.MTReference.context(TODAY())]);cards=longTermCards(longData);if(ctx)model=window.MTReference.buildModel(ctx);}else{
      const rows=await globalTrackerRows(28);adaptiveRows=rows;
      const sleepEnergy=relationTrend('Sommeil ↔ énergie','Les journées avec une durée de sommeil différente voient aussi l’énergie renseignée évoluer',pairByDate(rows,'sommeil_profond',crossSleep,['performance_recuperation','cycle','perimenopause','stress_regulation'],crossEnergy));if(sleepEnergy)cards.push(['Récupération',sleepEnergy]);
      const walkMood=relationTrend('Marche ↔ humeur','Les journées avec davantage de pas voient aussi l’humeur renseignée évoluer',pairByDate(rows,'pas_marche',v=>num(v.steps),['stress_regulation','cycle','perimenopause'],v=>num(v.mood)));if(walkMood)cards.push(['Mental',walkMood]);
      const stressSleep=relationTrend('Stress ↔ sommeil','Le stress renseigné et la qualité de sommeil semblent évoluer ensemble',pairByDate(rows,'stress_regulation',v=>num(v.stress),['sommeil_profond'],v=>num(v.quality)));if(stressSleep)cards.push(['Mental',stressSleep]);
      const fiberDigestion=relationTrend('Fibres ↔ digestion','Les journées avec une quantité de fibres différente voient aussi ton confort digestif renseigné évoluer',pairByDate(rows,'nutrition_vegetale',v=>num(v.fiber_g),['digestion','nutrition_vegetale'],v=>num(v.comfort)??num(v.digestion)));if(fiberDigestion)cards.push(['Nutrition',fiberDigestion]);
      const proteinSatiety=relationTrend('Protéines ↔ satiété','Les journées avec une quantité de protéines différente voient aussi la satiété après les repas évoluer',pairByDate(rows,'nutrition_vegetale',v=>num(v.protein_g),['equilibre_alimentaire'],v=>num(v.satiety_after)));if(proteinSatiety)cards.push(['Nutrition',proteinSatiety]);
      const activityRecovery=relationTrend('Mouvement ↔ récupération','Les journées plus ou moins actives voient aussi la récupération renseignée évoluer',pairByDate(rows,'pas_marche',v=>num(v.steps),['performance_recuperation'],v=>num(v.recovery)));if(activityRecovery)cards.push(['Récupération',activityRecovery]);
      const temporal=temporalRefluxTrend(rows);if(temporal)cards.push(['Confort',temporal]);
      if(window.MTReference?.context){const ctx=await window.MTReference.context(TODAY());if(ctx)model=window.MTReference.buildModel(ctx);}
    }
    const personal=model?`<div class="mt-global-trend"><small>REPÈRE PERSONNEL</small><b>${esc(window.MTReference.statusLabel(model))}</b><p>${esc(days===90?'Le long terme s’appuie sur des mois agrégés ; les détails journaliers ne sont pas retéléchargés.':`${Number(model.summary?.documented_days)||0} journées documentées sur 28 jours. Tes repères évoluent seulement quand plusieurs journées deviennent comparables.`)}</p></div>`:'';
    const adaptiveDecision=model&&window.MTAdaptive?.build?window.MTAdaptive.build(model,{rows:adaptiveRows,mode:days===90?'90d':'28d'}):null;
    const adaptiveHTML=adaptiveDecision&&window.MTAdaptive?.render?window.MTAdaptive.render(adaptiveDecision,{variant:'trend'}):'',holisticHTML=model?holisticTrendHTML(model):'';
    modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">Méthode Tee · transversal</div><h2><span class="mt-simple-star-inline" aria-hidden="true">✷</span>Mes tendances</h2>${trendPeriodButtons(days)}<p class="mt-follow-intro">${days===90?'Vue longue durée : les tendances sont calculées à partir d’agrégats mensuels compacts. La priorité adaptative reste volontairement fondée sur les 28 derniers jours comparables.':'Corps · Nutrition · Cycle · Confort · Mental. Cette vue relie seulement les journées réellement comparables.'}</p><div class="mt-global-trends">${personal}${adaptiveHTML}${holisticHTML}${cards.length?cards.map(([cat,x])=>`<div class="mt-global-trend"><small>${esc(cat)}</small><b>${esc(x.title)}</b><p>${esc(x.copy)}</p></div>`).join(''):'<div class="mt-global-trend"><small>PREMIERS REPÈRES</small><b>Ton historique se construit</b><p>Il faut encore quelques journées comparables avant de faire apparaître une tendance personnelle fiable. Continue simplement à renseigner ce qui t’est utile.</p></div>'}</div><div class="mt-follow-help">Méthode Tee observe, relie, choisit un seul levier prioritaire puis réévalue. Les tendances montrent des associations dans tes données, pas des causes certaines. Une absence reste absente et n’est jamais transformée en zéro.</div></section>`;
  };
  ...
  function friendlyTrackerSource(key){if(/^protocol_tracker[:._-]/i.test(String(key||'')))return 'Suivi du protocole';return ({food_day:'Ma journée alimentaire',daily_activity:'Carnet',journal:'Journal',beverages:'Boissons',sommeil_profond:'Sommeil approfondi',stress_regulation:'Stress & régulation',digestion:'Confort digestif',reflux:'Reflux & aigreurs',equilibre_alimentaire:'Équilibre alimentaire',evolution_corporelle:'Évolution corporelle',peau:'Peau',performance_recuperation:'Activité & récupération',cycle:'Cycle & rythme hormonal',perimenopause:'Périménopause & ménopause',protocol_tracker:'Suivi du protocole',jeune_intermit:'Jeûne intermittent',reduction_sucre:'Réduction du sucre',fringales_envies:'Fringales & envies',nutrition_vegetale:'Nutrition & micronutriments',pas_marche:'Pas & marche',changer_habitude:'Changer une habitude'})[key]||key;}
  ...
  const PROTOCOL_RELATIONS={
    ...
    fringales_envies:/sucre|glycem|stress|cortisol|jeune|nutrition|silhouette|equilibre|cycle|hormon|menstru|perimenopause|menopause/i,
  ...
  function appDerivedValues(rawKey,day,previousDay,context,date){
    ...
    if(key==='sommeil_profond'){
      ...
      // Les sensations au moment de l'envie restent une saisie explicite. Le reste du terrain
      // est relié en arrière-plan sans préremplir ni écraser la personne.
      put('_stress',stress);put('_fatigue',pn('tracker_fatigue'));put('_mood',mood);put('_energy',energy);
      put('_protein_g',c.protein_g);put('_fiber_g',c.fiber_g);put('_sugars_g',c.sugars_g);put('_food_energy_kcal',c.food_energy_kcal);put('_food_meal_count',c.food_meal_count);put('_first_meal_time',c.first_meal_time);put('_last_meal_time',c.last_meal_time);
      put('_previous_sleep_hours',previousDay?.core?.sleep_hours);put('_previous_sleep_quality',previousDay?.core?.sleep_quality);put('_recovery',c.recovery);put('_steps',c.steps);put('_active_energy_kcal',c.active_energy_kcal);
      put('_perimenopause_hot_flashes_context',nn('perimenopause.hot_flashes_context'));put('_perimenopause_night_sweats_context',nn('perimenopause.night_sweats_context'));
      const cyclePref=preference('cycle')?.settings||{},estimate=cycleEstimate(cyclePref,date);if(estimate)put('_cycle_phase',estimate.phase);
    }else if(key==='nutrition_vegetale'){
  ...
  function fillConnectedField(name,value){
    ...
    if(input.type==='range'){const output=input.nextElementSibling,denom=input.dataset.rangeDenom||input.max||10;if(output)output.value=`${input.value}/${denom}`;}
    return true;
  }
  function connectedHeadlineBits(key,values={}){
    ...
    else if(key==='fringales_envies'){if(present(values._food_meal_count))bits.push(`${values._food_meal_count} repas renseigné${Number(values._food_meal_count)>1?'s':''}`);if(present(values._previous_sleep_hours))bits.push(`nuit précédente ${fmtMetric(values._previous_sleep_hours,'h',1)}`);if(present(values._stress))bits.push(`stress ${values._stress}/10`);}
  ...
  function connectedBridgeHTML(){return `<div class="mt-app-connected" data-app-connected><small>REPÈRES DU JOUR</small><b>Synchronisation en cours</b><p>Méthode Tee rassemble les informations utiles déjà renseignées pour cette journée.</p></div>`;}
  async function hydrateAppConnectedForm(key,date){
    const box=document.querySelector('[data-app-connected]');if(!box)return null;const from=addDays(date,-1),context=await fetchAppConnectedContext(key,from,date);if(!context){box.innerHTML=`<small>REPÈRES DU JOUR</small><b>Ton suivi est prêt</b><p>Renseigne simplement ce qui correspond à ta journée.</p>`;return null;}
    ...
    const sugarNote=key==='reduction_sucre'&&present(values._documented_sugars_g)?'<p><strong>À savoir :</strong> les sucres affichés correspondent aux sucres totaux documentés, pas nécessairement aux sucres ajoutés.</p>':'';
    if(bits.length||sources.length||protocols.length){box.innerHTML=`<small>REPÈRES DU JOUR</small><b>${bits.length?esc(bits.join(' · ')):'Tes informations sont à jour'}</b><p>Les informations utiles déjà renseignées dans Méthode Tee sont intégrées automatiquement à ce suivi.</p>${sugarNote}`;}else box.innerHTML=`<small>REPÈRES DU JOUR</small><b>À compléter aujourd’hui</b><p>Renseigne les éléments qui correspondent à ta journée. Méthode Tee utilisera automatiquement les informations déjà disponibles lorsqu’elles sont pertinentes.</p>`;
  ...
  async function fetchHistory(rawKey,days=7,force=false){
    ...
    appRows.forEach(row=>{if(key!=='fringales_envies'||manual.has(row.entry_date))merged.set(row.entry_date,row);});
  ...
  function primarySpec(key){return ({
    stress_regulation:{label:'niveau de stress',good:-1,get:v=>num(v.stress)},
    ...
    fringales_envies:{label:'présence des envies',good:0,get:v=>num(v.urge_presence)??num(v.urge_intensity)},
  ...
  function cautiousRelation(key,rows){
    const map={
      stress_regulation:['qualité du sommeil','stress ressenti',v=>num(v.sleep_context),v=>num(v.stress)],
      ...
      fringales_envies:['faim physique','intensité des envies',v=>num(v.physical_hunger),v=>num(v.urge_intensity)],
  ...
  function ratingCard(label,rows,get,source='Ressenti personnel',max=10){const series=metricSeries(rows,get);if(!series.length)return null;const value=average(series.map(x=>x.value)),denom=Math.max(1,Number(max)||10);return {type:'rating',label,value:`${fmtMetric(value,'',1)} / ${denom}`,detail:`Moyenne sur ${series.length} repère${series.length>1?'s':''}`,source,percent:Math.max(0,Math.min(100,value/denom*100)),aria:`${label} : moyenne ${fmtMetric(value,'',1)} sur ${denom}`};}
  ...
  function trackerAnalytics(key,rows,settings={},allRows=rows){
    ...
    if(key==='sommeil_profond'){
      ...
      add(ratingCard('Présence des envies',rows,v=>num(v.urge_presence),'Ressenti personnel',5));add(ratingCard('Intensité de l’envie',rows,v=>num(v.urge_intensity),'Ressenti personnel',5));add(ratingCard('Faim physique',rows,v=>num(v.physical_hunger),'Ressenti personnel',5));add(ratingCard('Apaisement après avoir mangé',rows,v=>num(v.apaisement_after),'Ressenti personnel',5));add(frequencyCard('Jours avec envie présente',rows,v=>num(v.urge_presence)!==null&&num(v.urge_presence)>1));add(frequencyCard('Collations structurées',rows,v=>/Collation prévue \/ structurée/.test(String(v.eating_response||''))));add(distributionCard('Contextes les plus renseignés',rows,v=>v.main_context));add(distributionCard('Réponses les plus renseignées',rows,v=>v.eating_response));add(distributionCard('Types d’envies',rows,v=>v.urge_type));
    }
    else if(key==='reduction_sucre'){
  ...
  function historyHighlights(key,rows){
    ...
    else if(key==='fringales_envies'){const urges=values.map(v=>num(v.urge_intensity)).filter(Number.isFinite),presence=values.map(v=>num(v.urge_presence)).filter(Number.isFinite),hunger=values.map(v=>num(v.physical_hunger)).filter(Number.isFinite);if(presence.length)push(Math.round(average(presence)*10)/10,'présence moyenne /5');if(urges.length)push(Math.round(average(urges)*10)/10,'intensité moyenne /5');if(hunger.length)push(Math.round(average(hunger)*10)/10,'faim physique moyenne /5');push(values.filter(v=>/Collation prévue \/ structurée/.test(String(v.eating_response||''))).length,'collations structurées renseignées');}
  ...
  function chartSpecs(key,settings={}){const map={
    ...
    fringales_envies:[['Présence des envies',v=>num(v.urge_presence),'/5'],['Intensité de l’envie',v=>num(v.urge_intensity),'/5'],['Faim physique',v=>num(v.physical_hunger),'/5'],['Apaisement',v=>num(v.apaisement_after),'/5']],
  ...
  function metricListHTML(key,rows,settings={}){
    ...
    return `<section class="mt-health-summary"><div class="mt-health-summary-head"><h3>Mes repères</h3><small>Dernière donnée enregistrée<br>touche un repère pour voir son évolution</small></div><div class="mt-health-metrics">${cards.join('')}</div></section>`;
  }
  function metricAboutCopy(key,label){
    ...
    if(key==='sommeil_profond')return `${label} peut utiliser les données de sommeil déjà renseignées dans Méthode Tee. Apple Santé peut compléter certaines mesures objectives. Le ressenti personnel reste distinct des mesures automatiques.`;
  ...
  function renderHistory(modal,key,rows,days=7){
    ...
    modal.dataset.key=key;modal.dataset.period=String(days);modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" type="button" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">${esc(item.title)} · évolution</div><h2>Analyse complète</h2><div class="mt-follow-history-head"><button class="mt-follow-period ${days===7?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(7)">7 j</button><button class="mt-follow-period ${days===28?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(28)">28 j</button><button class="mt-follow-period ${days===90?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(90)">90 j</button><button class="mt-follow-period ${!Number(days)?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(0)">Depuis le début</button><button class="mt-follow-period" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">+ Aujourd’hui</button></div>${analyticsHTML(key,shown,settings,days,rows)}${premiumInsightHTML(key,shown,settings,days)}${curvesHTML(key,shown,settings)}<div class="mt-follow-history-stats"><div class="mt-follow-stat"><b>${stats.all?stats.count:`${stats.count}/${days}`}</b><small>${stats.all?'repères renseignés':'jours renseignés'}</small></div><div class="mt-follow-stat"><b>${stats.average===null?'—':String(Math.round(stats.average*10)/10).replace('.',',')}</b><small>${esc(stats.label)}</small></div></div>${highlightHTML}<div class="mt-follow-insight"><small>Lecture prudente · ${esc(periodLabel)}</small><b>${esc(stats.trend)}</b><p>${esc(stats.relation||'Continue quelques repères comparables pour faire ressortir une tendance personnelle utile.')} ${esc(used)}</p></div><div class="mt-follow-insight" data-cross-tracker-insight hidden></div>${key==='evolution_corporelle'?'<div class="mt-follow-source-note">Les variations de poids, mensurations ou composition corporelle sont décrites sans jugement. Compare de préférence des mesures prises dans des conditions proches.</div>':''}${historyRowsHTML(key,shown)}<button class="mt-follow-secondary" type="button" onclick="mtAdvancedTrackerGlobalTrends(28)"><span class="mt-simple-star-inline" aria-hidden="true">✷</span>Ouvrir Mes tendances</button><button class="mt-follow-secondary" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">Renseigner aujourd’hui</button></section>`;
  ...
  async function loadCrossTrackerInsight(modal,key,days){
    const host=modal.querySelector('[data-cross-tracker-insight]');if(!host)return;
    if(key==='fringales_envies'&&window.MTReference?.context){
      const ctx=await window.MTReference.context(TODAY()),cr=ctx?.holistic?.cravings||{},high=Number(cr.high_days)||0;if(high<3)return;
      const choices=[['des nuits précédentes plus courtes',Number(cr.high_after_shorter_sleep)||0],['un stress plus élevé',Number(cr.high_with_high_stress)||0],['des protéines documentées plus basses',Number(cr.high_with_lower_protein)||0],['des fibres documentées plus basses',Number(cr.high_with_lower_fiber)||0]].sort((a,b)=>b[1]-a[1]),best=choices[0];
      let copy='Tes envies marquées commencent à former un historique comparable. Continue à noter la faim physique et le contexte avant de chercher à les modifier.';
      if(best?.[1]>=Math.max(2,Math.ceil(high*.6)))copy=`Sur tes journées comparables avec envie marquée, ${best[0]} revient souvent dans le même contexte. C’est une piste personnelle à observer, pas une preuve de cause.`;
      else if(Number(cr.meal_gap_days)>=3&&Number(cr.avg_minutes_since_previous_meal)>=240)copy='Plusieurs envies marquées ont été renseignées après un intervalle d’au moins quatre heures depuis le repas précédent. Cela peut correspondre à une vraie faim : une collation adaptée n’est pas considérée comme un échec.';
      host.hidden=false;host.innerHTML=`<small>Ce que j’observe · contexte relié</small><b>${high} journée${high>1?'s':''} avec envie marquée</b><p>${esc(copy)}</p>`;return;
    }
    let keys=[],xKey='',yLabel='',xLabel='',xGetter=null,yGetter=null;
  ...
  function notePrompt(key){
    return ({
      ...
      fringales_envies:['Observation libre (facultatif)','Un détail sur le moment, l’environnement ou ce que tu as ressenti…'],
  ...
  function entryFeedback(key,values={},settings={}){
    ...
    let result;
    if(key==='stress_regulation'){
      const stress=num(values.stress),help=String(values.regulation_help||''),load=num(values.mental_load);
      if(stress!==null&&stress>=7)result=['Alléger ce qui peut l’être','Ton stress est élevé aujourd’hui. Garde une seule action de régulation réaliste et observe surtout l’évolution sur plusieurs jours.'];
      else if(/Oui, clairement|Un peu/.test(help))result=['Garder ce qui t’aide','La stratégie renseignée semble avoir été utile aujourd’hui. Répète-la quand le même contexte revient et compare tes repères.'];
      else if(load!==null&&load>=7)result=['Protéger un espace de récupération','Ta charge mentale est élevée. Un petit moment sans sollicitations peut devenir un repère à comparer, sans objectif de performance.'];
      else result=['Construire ton propre repère','Continue à noter stress, sommeil et ce qui t’aide. Les tendances personnelles deviennent plus utiles avec plusieurs journées comparables.'];
    }
    else if(key==='sommeil_profond'){
      ...
      const presence=num(values.urge_presence),urge=num(values.urge_intensity),hunger=num(values.physical_hunger),response=String(values.eating_response||'');
      if(/Collation prévue \/ structurée/.test(response))result=['Une collation peut être un vrai repère','Une collation adaptée à ta faim et à ton rythme n’est pas un échec. Observe surtout si elle répond à ton besoin et comment tu te sens ensuite.'];
      else if(urge!==null&&urge>=4&&hunger!==null&&hunger>=4)result=['La faim physique mérite d’être entendue','L’envie et la faim physique sont toutes les deux marquées. Avant de chercher à supprimer ce moment, laisse Méthode Tee comparer l’espacement et la structure de tes repas sur plusieurs journées.'];
      else if(urge!==null&&urge>=4&&hunger!==null&&hunger<=2)result=['Continuer à relier sans jugement','L’envie est marquée alors que la faim physique est plus faible aujourd’hui. L’heure et le contexte aideront à comprendre ce qui se répète, sans attribuer de cause après une seule journée.'];
      else if(presence!==null&&presence<=1)result=['Un jour sans envie compte aussi','Les journées où les envies sont absentes ou discrètes sont utiles : elles permettent de comparer ce qui change réellement dans ton rythme.'];
      else result=['Construire ton propre repère','Continue quelques jours à noter présence, intensité, faim physique et contexte. Méthode Tee les reliera aux données déjà disponibles sans te demander de les ressaisir.'];
    }
    else if(key==='reduction_sucre'){
  ...
  const TODAY_CARD_ICONS={stress_regulation:'sparkle',sommeil_profond:'sleep',digestion:'bowl',reflux:'sparkle',equilibre_alimentaire:'bowl',evolution_corporelle:'chart',peau:'sparkle',performance_recuperation:'movement',cycle:'chart',perimenopause:'sparkle',jeune_intermit:'clock',reduction_sucre:'sparkle',fringales_envies:'sparkle',nutrition_vegetale:'leaf',pas_marche:'movement',changer_habitude:'seed'};
  ...
  async function todayTrackerCards(){
    ...
    return active.map(key=>{
      ...
      const derived=row?{headline:'',hasData:false,source:null,derived:false}:(key==='fringales_envies'?{headline:'',hasData:false,source:null,derived:false}:(key==='pas_marche'?(legacyDerived.hasData?legacyDerived:appDerived):(appDerived.hasData?appDerived:legacyDerived)));
