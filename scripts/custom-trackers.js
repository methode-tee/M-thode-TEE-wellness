...
(function(){
  ...
  function micronutrientCard(key,rows){const meta=MICRO_META[key];if(!meta)return null;const [label,unit]=meta,series=metricSeries(rows,v=>microNumber(v,key));if(!series.length){const latest=[...rows].find(r=>microSources(r.values||{},key).length);const names=latest?microSources(latest.values||{},key):[];return statusCard(label,'Non documenté',names.length?`Sources renseignées : ${names.slice(0,3).join(' · ')}`:'Aucune source calculable renseignée sur cette période. Cela ne signifie pas une carence.','Carnet · CIQUAL');}const vals=series.map(x=>x.value),latest=series.at(-1),latestRow=[...rows].find(r=>r.entry_date===latest.date),sources=latestRow?microSourceDetails(latestRow.values||{},key,unit):[];return {type:'range',label,value:`${fmtMetric(Math.min(...vals),unit,2)}${Math.min(...vals)===Math.max(...vals)?'':` – ${fmtMetric(Math.max(...vals),unit,2)}`}`,detail:`Dernière donnée : ${fmtMetric(latest.value,unit,2)}${sources.length?` · Sources : ${sources.slice(0,3).join(' · ')}`:''}`,source:'Carnet · CIQUAL',min:Math.min(...vals),max:Math.max(...vals),latest:latest.value,aria:`${label}, plage ${fmtMetric(Math.min(...vals),unit,2)} à ${fmtMetric(Math.max(...vals),unit,2)}`};}
  function trackerAnalytics(key,rows,settings={},allRows=rows){
    ...
    if(key==='sommeil_profond'){
      ...
      if(selected.includes('protein'))add(rangeCard('Protéines calculées',rows,v=>num(v.protein_g),'g','Carnet · CIQUAL'));add(rangeCard('Glucides calculés',rows,v=>num(v.carbs_g),'g','Carnet · CIQUAL'));add(rangeCard('Lipides calculés',rows,v=>num(v.fat_g),'g','Carnet · CIQUAL'));if(selected.includes('fiber'))add(rangeCard('Fibres calculées',rows,v=>num(v.fiber_g),'g','Carnet · CIQUAL'));selected.filter(k=>MICRO_META[k]).forEach(k=>add(micronutrientCard(k,rows)));
      ...
      add(frequencyCard('Jours avec repas calculables',rows,v=>num(v.calculated_meals)>0,'Carnet · CIQUAL'));add(frequencyCard('Jours avec sources micronutritionnelles',rows,v=>num(v.micronutrient_source_count)>0,'Carnet · CIQUAL'));add(ratingCard('Satiété ressentie',rows,v=>num(v.satiety)));add(ratingCard('Impression de variété',rows,v=>num(v.variety)));add(ratingCard('Confort digestif',rows,v=>num(v.digestion)));
    }
  ...
  function chartSpecs(key,settings={}){const map={
    sommeil_profond:[['Durée',v=>num(v._sleep_hours)??durationBetween(v.bedtime,v.wake_time),'h'],['Qualité',v=>num(v.quality),'/10'],['Réveil',v=>num(v.wake_state),'/10']],
    digestion:[['Confort',v=>num(v.comfort),'/10'],['Ballonnements',v=>num(v.bloating),'/10'],['Douleurs',v=>num(v.pain),'/10']],
    reflux:[['Intensité',v=>/^Non/i.test(String(v.episode||''))?0:num(v.intensity),'/10']],
    equilibre_alimentaire:[['Diversité',v=>num(v.diversity),'/10'],['Protéines',v=>num(v.protein),'/10'],['Satiété',v=>num(v.satiety_after),'/10']],
    evolution_corporelle:[...(settings.hide_weight?[]:[['Poids',v=>num(v.weight),'kg']]),['Tour de taille',v=>num(v.waist),'cm'],['Confort corporel',v=>num(v.body_comfort),'/10']],
    peau:[['Inconfort',v=>avg(v.blemishes,v.dryness,v.inflammation,v.sensitivity,v.itching),'/10'],['Sensibilité',v=>num(v.sensitivity),'/10']],
    performance_recuperation:[['Récupération',v=>num(v.recovery),'/10'],['Charge ressentie',v=>num(v.intensity),'/10'],['Durée',v=>num(v.duration),'min']],
    cycle:[['Énergie',v=>num(v.energy),'/10'],['Douleurs',v=>num(v.pain),'/10'],['Sommeil',v=>num(v.sleep),'/10']],
    perimenopause:[['Énergie',v=>num(v.energy),'/10'],['Sommeil',v=>num(v.sleep),'/10'],['Stress',v=>num(v.stress),'/10']],
    jeune_intermit:[['Durée du jeûne',v=>num(v._fast_hours)??durationBetween(v.last_meal,v.first_meal),'h'],['Faim',v=>num(v.hunger),'/10'],['Énergie',v=>num(v.energy),'/10']],
    reduction_sucre:[['Envies',v=>num(v.craving),'/10'],['Faim',v=>num(v.hunger),'/10'],['Fatigue',v=>num(v.fatigue),'/10']],
    changer_habitude:[['Confiance',v=>num(v.confidence),'/10'],['Difficulté',v=>num(v.difficulty),'/10'],['Impulsion',v=>num(v.urge),'/10']],
    pas_marche:[['Pas',v=>num(v.steps),'pas'],['Distance',v=>num(v.distance_km),'km'],['Vitesse',v=>num(v.walking_speed_kmh),'km/h'],['Énergie active',v=>num(v.active_energy_kcal)??num(v._healthkit_active_energy_kcal),'kcal']],
    nutrition_vegetale:[['Protéines',v=>num(v.protein_g),'g'],['Fibres',v=>num(v.fiber_g),'g'],...(Array.isArray(settings.observed_nutrients)?settings.observed_nutrients:[]).filter(k=>MICRO_META[k]).slice(0,3).map(k=>[MICRO_META[k][0],v=>microNumber(v,k),MICRO_META[k][1]])]
  ...
  function renderHistory(modal,key,rows,days=7){
    ...
    modal.dataset.key=key;modal.dataset.period=String(days);modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" type="button" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">${esc(item.title)} · évolution</div><h2>Mes jauges & mes courbes</h2><div class="mt-follow-history-head"><button class="mt-follow-period ${days===7?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(7)">7 j</button><button class="mt-follow-period ${days===28?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(28)">28 j</button><button class="mt-follow-period ${days===90?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(90)">90 j</button><button class="mt-follow-period ${!Number(days)?'is-on':''}" type="button" onclick="mtAdvancedTrackerHistoryPeriod(0)">Depuis le début</button><button class="mt-follow-period" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">+ Aujourd’hui</button></div>${analyticsHTML(key,shown,settings,days,rows)}${curvesHTML(key,shown,settings)}<div class="mt-follow-history-stats"><div class="mt-follow-stat"><b>${stats.all?stats.count:`${stats.count}/${days}`}</b><small>${stats.all?'repères renseignés':'jours renseignés'}</small></div><div class="mt-follow-stat"><b>${stats.average===null?'—':String(Math.round(stats.average*10)/10).replace('.',',')}</b><small>${esc(stats.label)}</small></div></div>${highlightHTML}<div class="mt-follow-insight"><small>Lecture prudente · ${esc(periodLabel)}</small><b>${esc(stats.trend)}</b><p>${esc(stats.relation||'Continue quelques repères comparables pour faire ressortir une tendance personnelle utile.')} ${esc(used)}</p></div><div class="mt-follow-insight" data-cross-tracker-insight hidden></div>${key==='evolution_corporelle'?'<div class="mt-follow-source-note">Les variations de poids, mensurations ou composition corporelle sont décrites sans jugement. Compare de préférence des mesures prises dans des conditions proches.</div>':''}${historyRowsHTML(key,shown)}<button class="mt-follow-secondary" type="button" onclick="mtAdvancedTrackerEntry('${esc(key)}')">Renseigner aujourd’hui</button></section>`;
  ...
  async function hydrateInlineAnalytics(key){
    ...
    try{
      ...
      host.innerHTML=`<div class="mt-follow-analytics-title"><h3>Aperçu de mon évolution</h3><small>7 derniers jours · jauges & courbes visibles ici</small></div>${analyticsHTML(key,rows,settings,7,rows)}${curvesHTML(key,rows,settings)}<button class="mt-follow-secondary" type="button" onclick="mtAdvancedTrackerHistory('${esc(key)}')">Ouvrir l’analyse complète · 7 / 28 / 90 j</button>`;
