...
(function(){
  ...
  window.mtAdvancedTrackerGlobalTrends=async function(days=28){
    ...
    if(days===90&&window.MTReference?.overview){const [longData,ctx]=await Promise.all([window.MTReference.overview('3m'),window.MTReference.context(TODAY())]);cards=longTermCards(longData);if(ctx)model=window.MTReference.buildModel(ctx);}else{
      const rows=await globalTrackerRows(28);
    ...
    modal.innerHTML=`<div class="mt-follow-bg" onclick="mtAdvancedTrackerEntryClose()"></div><section class="mt-follow-sheet"><div class="mt-follow-grip"></div><button class="mt-follow-close" onclick="mtAdvancedTrackerEntryClose()">×</button><div class="mt-follow-kicker">Méthode Tee · transversal</div><h2><span class="mt-simple-star-inline" aria-hidden="true">✷</span>Mes tendances</h2>${trendPeriodButtons(days)}<p class="mt-follow-intro">${days===90?'Vue longue durée : les tendances sont calculées à partir d’agrégats mensuels compacts.':'Corps · Nutrition · Cycle · Confort · Mental. Cette vue relie seulement les journées réellement comparables.'}</p><div class="mt-global-trends">${personal}${cards.length?cards.map(([cat,x])=>`<div class="mt-global-trend"><small>${esc(cat)}</small><b>${esc(x.title)}</b><p>${esc(x.copy)}</p></div>`).join(''):'<div class="mt-global-trend"><small>PREMIERS REPÈRES</small><b>Ton historique se construit</b><p>Il faut encore quelques journées comparables avant de faire apparaître une tendance personnelle fiable. Continue simplement à renseigner ce qui t’est utile.</p></div>'}</div><div class="mt-follow-help">Les tendances montrent des associations dans tes données, pas des causes certaines. Une absence reste absente et n’est jamais transformée en zéro.</div></section>`;
  };
